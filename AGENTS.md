...
- Grants alone cannot do it. `scripts/init-db.sql` both `GRANT`s on **all** existing `public` tables and sets `ALTER DEFAULT PRIVILEGES ... ON TABLES` for every runtime role, so any per-role `REVOKE` a migration performs is silently re-widened the next time init-db runs; and no grant ever fences the **table owner** (the shared migration/dashboard login). A single-role `public` table therefore needs `ENABLE ROW LEVEL SECURITY` **plus** `FORCE ROW LEVEL SECURITY` (force is what binds the owner) with a policy keyed on `current_user = '<role>'`, on top of the revoke/grant pass. Test the fence by asserting the owner sees zero rows and is refused on insert, not just that another runtime role is denied.
...
- Rollback is **not** uniformly available across the core chain: `core_196` (restore-drill executor) and `core_198` (runtime-attention outbox) install trusted-bootstrap boundaries whose downgrade is bootstrap-only and may refuse outright, and later revisions can retain cutover state that makes `core_198`'s rollback permanently unavailable. A test that rolls back one old migration must therefore **bound its upgrade to the revision it owns** — `command.upgrade(cfg, f"core@{mod.revision}")`, or `create_migrated_test_db(..., revisions={"core": "core_NNN"})` — never `core@head` followed by a downgrade into the 130s-170s. Migrating to head first drags those boundaries into an unrelated rollback and fails with `core_198 downgrade requires trusted bootstrap rollback interface` (bu-2rqrl).
- Adding a core migration moves the chain head, which **breaks head-pinned test assertions**: several tests run `command.upgrade(cfg, "core@head")` and then compare `SELECT version_num FROM <schema>.alembic_version` against a hardcoded literal, so a correct new revision fails with `AssertionError: assert 'core_NNN' == 'core_<old>'`. Bump the literals as part of the migration bead — as of head `core_201` the sites are `tests/migrations/test_runtime_attention_outbox_migration.py:1242,1248,1300,1352` and `tests/migrations/test_runtime_attention_producer_upgrade.py:77` (plus the `core@head` test ~:185). Do **not** rewrite sites that legitimately name a specific revision: `down_revision == "core_NNN"` checks, contract tests asserting a revision's own filename/`revision ==` string, and docstrings. Root-cause fix (resolve head from the script directory) tracked as bu-4sgl8.
