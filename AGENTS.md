...
- Owner Telegram handle seeding is relationship-schema-only: `_seed_owner_telegram_handle` must check `current_schema() = 'relationship'` before probing `relationship.entity_facts`, because other butler runtime roles cannot resolve that schema and should not log startup warnings for a relationship-only seed.
