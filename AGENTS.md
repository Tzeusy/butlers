...
- CI Python work is a fan-out/fan-in: `check-unit` and `check-integration` write distinct non-hidden coverage data artifacts, then the legacy `check` job runs after failed or successful lanes (except a cancelled workflow) and explicitly fails if either lane did not succeed before combining them. Never use `--cov-append` as cross-job handoff; it is valid only for the sequential local `make test-ci-*` targets. The durable test evidence is sanitized JUnit metadata plus top-duration tables, never raw failure/stdout payloads.
