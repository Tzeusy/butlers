...
- The guarded core integration modules (`tests/core/test_core_state.py`, `tests/core/test_core_sessions.py`, and `tests/core/test_core_scheduler.py`) that create asyncpg pools in async fixtures must apply session asyncio loop scope to each async test function (via `@pytest.mark.asyncio(loop_scope="session")` or a local alias) under xdist; do not apply the mark to a whole module or class because synchronous guards may be collected there.
