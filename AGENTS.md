...
- GPT-5.6 Spend prices intentionally use OpenAI Standard API-equivalent metered rates even when the runtime is subscription-covered; pricing lookup is exact, so live `gpt-5.6-luna-high`/`-xhigh` catalog IDs need their own entries. The long-context tiers require `context_tokens`, but ledger aggregates retain only token buckets and therefore use the <=272K fallback until per-request context is persisted (bu-hbo6x).
