...
- Entity-dedup curation treats a matching ordered pair with a `pending`, `approved`, or `rejected` `memory_entity_merge`/legacy `entity_merge` row as existing lifecycle state. Only expired actions may resurface; curation must never retry or mutate an approved historical merge.
...
- Entity-dedup curation uses `relationship:entity-dedup:<source>:<target>` as `pending_actions.deduplication_key`; `approvals_012` uniquely protects `pending`/`approved`/`rejected` rows, preserves NULL historic rows, and permits expired pairs to resurface. Select the surviving target by `(created_at, id) ASC`.
