...
- Test-condensation counts must use `^[[:space:]]*(async[[:space:]]+)?def[[:space:]]+test_` so async and class-indented tests count; keep historical phase figures separate from the current exact CI-selection measurement.
