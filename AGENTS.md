...
- Capture `headRefOid`, the live target-branch ref name, and its live ref SHA (not the PR's potentially stale `baseRefOid`) together during the final terminal-CI / independent-review revalidation and use `python3 scripts/merge_pr_exact_base.py --pr <n> --expected-head <head> --expected-base-ref <ref> --expected-base <base>` rather than a bare REST merge. The helper keeps SHA pinning, rejects pre-request head/target-ref/base drift without issuing a merge, then re-reads the merged PR's retained `baseRefName` through GraphQL before auditing the resulting squash commit's sole parent and immutable result tree.
- Only `merged-exact-base` (`source_bead_closure_allowed: true`) permits source/review Bead closure. `premerge-head-drift`, `premerge-base-ref-drift`, or `premerge-base-drift` requires a rebase onto current `origin/main` followed by fresh CI and independent exact-head review. `postmerge-base-ref-drift` (including a failed post-merge GraphQL lookup), `postmerge-base-drift`, and `postmerge-patch-drift` are nonzero, already-merged race classifications: keep the source Bead open and require a documented post-merge audit; never portray any as exact-current-base evidence. For patch proof, the audit must record matching `expected_patch_tree_sha` / `landed_patch_tree_sha`: coupled with the verified sole parent equal to the reviewed base, immutable tree equality proves the same net patch without relying on local Git state.
...
- `messenger.delivery_requests`/`delivery_attempts`/`delivery_receipts`/`delivery_dead_letter` (schema + `IdempotencyEngine` in `roster/messenger/tools/idempotency.py`, dead-letter list/inspect/replay/discard tools in `roster/messenger/tools/delivery/dead_letter.py`) is fully-built but NEVER WIRED to the live send path (verified bu-ep4ks.4, 2026-07-26): `grep -rn "delivery_requests" src --include="*.py"` outside tests returns zero hits. The actual live messenger delivery path is `src/butlers/core_tools/_routing.py::_route_execute_inner`'s inline synchronous branch (`daemon.config.name == "messenger"`), which calls the channel module directly (`telegram_module._send_message`, etc.) and never touches these tables — so in production the tables are empty, `messenger_dead_letter_replay` has nothing to act on (and even if it did, it only INSERTs a new `pending` row; no consumer process reads `pending` rows and actually re-sends), and `messenger_delivery_stats`/`queue_depth` report zeros. The REAL live record of every notify() delivery outcome (including terminal failures) is `switchboard.notifications` (written by `log_notification()`, called from `deliver.py::_deliver_via_notify_request`) plus `public.attention_ledger` (written by `record_attention_event()` at the notify() boundary in `core_tools/_notifications.py`, linked via `notification_ref` = `notifications.id`). Build delivery-observability/retry features against `switchboard.notifications`/`attention_ledger`, not the messenger delivery-tracking schema, until/unless a future bead actually wires admission into `_route_execute_inner` (a large, separate-scope change touching the live send path for every butler).
