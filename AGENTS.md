...
- Switchboard migrations currently run through `sw_007`; new switchboard revisions must continue linearly from `sw_007` to avoid multi-head failures during `switchboard@head` upgrades.
- `fanout_execution_log` schema is migration-managed (`roster/switchboard/migrations/007_create_fanout_execution_log.py`); runtime routing code should remain insert-only and must not issue `CREATE TABLE` DDL.
