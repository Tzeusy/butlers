...
- Dashboard chat Stop is message-scoped, not conversation-scoped: propagate the immutable dashboard user-message id through ingress, route inbox, recovery, and `Spawner`, and only render cancellation after the durable control row confirms it. Route-inbox workers must use fenced processing leases so a stale recovery cannot overwrite a newer worker's result.
