...
- `owntracks.ssid_presence` checkpoints use a source-scoped `_source_cursor` in `projection_checkpoints.carryover` to pair the timestamp watermark with the stable OwnTracks row UUID. Tuple paging is `(ts, id) > (...)`; a legacy timestamp-only or mismatched cursor triggers a bounded replay from retained evidence with carryover rebuilt from scratch, because replaying only the boundary can double-count rows already present in the open SSID span. The first successful page writes the composite cursor and marks the upgrade complete; mapping-replay write atomicity remains tracked separately in bu-whhll.16.
