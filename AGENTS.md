...
- `_seed_owner_telegram_handle` is relationship-schema-only: check `current_schema() = 'relationship'` before probing `relationship.entity_facts`; non-relationship runtime roles lack privileges and should no-op without startup warnings.
...
- `GmailPolicyEvaluator` reads `public.priority_contacts` joined to `relationship.entity_facts` under `SET ROLE connector_writer`; relationship migrations must grant `connector_writer` `USAGE` on schema `relationship` plus `SELECT` on `relationship.entity_facts`, but no write privileges.
