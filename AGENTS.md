...
### Beads worktree export guardrail
- In git-worktree direct mode (`BEADS_NO_DAEMON=1`), `bd create`/`bd update` changes may exist in the local DB before appearing in `.beads/issues.jsonl`; run `bd export -o .beads/issues.jsonl` before commit/push to ensure new/updated issues are persisted to git-tracked JSONL.
