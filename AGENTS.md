...
- The Dolt server backing `bd` for this rig is **Gas Town's shared server on port 3307** (`gt dolt start`, data dir `~/gt/.dolt-data`, serves all rig DBs). Do NOT `bd dolt start` to recover it — that squats 3307 with an empty local server rooted at `.beads/dolt`, after which bd reports `database "butlers" not found`. Fix: `bd dolt stop` (kill the local squatter) then `gt dolt start`; verify with `gt dolt status`. If the `butlers` DB was DROP'd (lives in `.beads/dolt/.dolt_dropped_databases/`), `CALL dolt_undrop('butlers')` via `mysql -h127.0.0.1 -P3307 -uroot` restores it. `gt dolt status` lists DBs from the on-disk data dir, so it can show DBs that aren't actually served by whatever is live on 3307 — confirm liveness with `mysql ... -e 'show databases'`.
- CI has a `guard-direct-push` job that FAILS any `main` commit not arriving via a merged PR. Route EVERY change through a GitHub PR — even trivial/direct-merge-candidate beads — never `git push origin main` ff-merges, or `guard-direct-push` goes red (code is fine but the run is red). It also sometimes races on squash-merge PR-association. The standing fix is enabling branch protection (owner-gated bead). When triaging a red `main` run, read JOB-BY-JOB (`gh run view <id> --json jobs`): `check`/`frontend`/`frontend-e2e` are the code-health jobs; a lone `guard-direct-push` failure is policy noise, not broken code. Conversely, do NOT trust a reviewer's "pre-existing" claim without the job-by-job check — a real frontend-build break was once dismissed and merged red because adding a required field to a shared TS type (e.g. `ConnectorDetail`) left `.test.tsx` fixtures non-compiling (`npm run build`/`tsc -b` catches it; `tsc --noEmit` on src does not).
- Telegram identifiers in `relationship.entity_facts` `has-handle` triples are canonically `telegram:<id>` (the prefix disambiguates telegram from other `has-handle` channels like linkedin/twitter). All write sites (reconciler `relationship_jobs.py`, `backfill_contact_info_triples.py`, `contact_info_add`) and read sites (`daemon._resolve_contact_channel_identifier`, `_ef_channel_helpers.py`, which filter `LIKE 'telegram:%'`) must agree — use `_ef_channel_helpers.encode_handle_object()`. A verbatim (unprefixed) write silently breaks `notify(channel='telegram')`.
