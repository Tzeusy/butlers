"""Create calendar.v_overlay_contributions cross-schema view.

Revision ID: core_140
Revises: core_139
Create Date: 2026-06-21 00:00:00.000000

Creates a read-only SQL view ``calendar.v_overlay_contributions`` that unions
calendar-overlay contribution rows from each contributing specialist butler's
``state`` table.  This is a sanctioned exception to schema isolation (RFC 0006),
reusing the RFC 0010 Cross-Butler Briefing Exception under RFC-0020's accepted
criteria — mirroring ``general.v_briefing_contributions`` (migration ``core_063``).

The view is consumed by the calendar workspace ``view=overlays`` read path, which
projects precomputed overlay envelopes into the unified calendar without any LLM
session or on-demand cross-schema fan-out (RFC-0020 Decision: no-LLM variant).

The five RFC 0010 guardrails are encoded structurally:
  ...
  3. Key-filtered — each term filters ``key LIKE 'calendar/overlay/%'``, bounding
     access to overlay keys only rather than the whole ``state`` table.
...
Columns exposed:
  ...
  key     TEXT   — state key (filtered to ``calendar/overlay/%``)
  value   JSONB  — overlay contribution envelope
"""
...
revision: str = "core_140"
down_revision: str = "core_139"
branch_labels = None
depends_on = None

# Specialist schemas whose state tables contribute to the overlay view.
# Must match the contributing set in the overlay aggregation spec
# (calendar-overlay-aggregation): finance, travel, relationship, health.
...
# query it.  Neither is created by core_001 (calendar is a module, not a
# stand-alone butler schema), so both are provisioned best-effort here.
_CALENDAR_SCHEMA = "calendar"
_CALENDAR_ROLE = "butler_calendar_rw"

_VIEW_FQN = f"{_CALENDAR_SCHEMA}.v_overlay_contributions"

# Key prefix bounding cross-schema access to overlay contributions only.
_KEY_PREFIX = "calendar/overlay/%"


def _ensure_role_exists(role_name: str) -> None:
    """Create role if it doesn't exist (best-effort, matches core_063 pattern)."""
    op.execute(f"""
        DO $$
        BEGIN
...
def upgrade() -> None:
    ...
    # Neither is provisioned by core_001 (calendar is a module surface, not a
    # stand-alone butler schema), so create them best-effort here.
    ...
    #    RFC 0010 guardrail 3: key filter bounds access to overlay keys only.
