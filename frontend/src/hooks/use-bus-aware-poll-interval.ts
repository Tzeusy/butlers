 ...
 *
 * This hook reads the shared EventBusProvider's health
 * (useEventBus, see lib/event-bus.tsx) and switches cadence on it:
 *   - "healthy": POLL_BUS_RECONCILE_MS — the bus is doing the real-time work;
 *     polling is only a reconciliation sweep.
 *   - "late" or "down": `fallbackMs` — a fast fallback so the surface
 *     degrades to honest polling instead of silently going stale for the full
