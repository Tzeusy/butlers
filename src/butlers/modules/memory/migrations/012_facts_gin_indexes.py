...
that approach so the planner can use the index for predicate scans:
  - 'meal_'    <= predicate < 'meal`'   (backtick is ASCII 96, one above '_' 95)
...
def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS idx_facts_measurement_predicate_valid")
    op.execute("DROP INDEX IF EXISTS idx_facts_transaction_predicate_valid")
    op.execute("DROP INDEX IF EXISTS idx_facts_meal_predicate_valid")
