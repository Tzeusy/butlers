...
|---|---|---|
| `DATABASE_URL` | -- | All (libpq-style URL; daemon publisher pools and the fleet bridge use its decoded database path as the target when set) |
| `POSTGRES_DB` | caller-configured fallback | Daemon publisher pools and fleet bridge (database target when `DATABASE_URL` is unset) |
