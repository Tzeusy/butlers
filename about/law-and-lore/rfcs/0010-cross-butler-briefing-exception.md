...
- **RFC 0009:** The Situational Context Bus (shared.user_context) uses a different mechanism -- a shared schema table readable by all butlers. That pattern does not require this exception because shared schema tables are already within every butler's search_path by design. The briefing exception is distinct: it reads from per-butler schemas, which are normally inaccessible.
...
**Shared briefing table (shared.briefing_contributions).** All specialists write to a table in the shared schema. Rejected because it introduces write coupling to the shared schema, which is currently read-only for most butlers (RFC 0006). It would also require extending the shared schema access model, creating precedent for arbitrary shared tables.
