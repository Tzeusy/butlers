...
| [0022](rfcs/0022-cross-process-event-transport.md) | Cross-Process Event Transport (NOTIFY/LISTEN Fleet Event Bridge) | Daemon-originated session/spend/notification/approval events are published via Postgres `pg_notify` from the `butlers-up` container's own DB pool; a dashboard-api-side `LISTEN` bridge re-publishes them onto the existing in-process fleet event bus (`WS /api/events/stream`) unchanged. Fixes daemon-originated live events being silently discarded at the container boundary (bu-01r64) — additive transport; does not replace the in-process bus's existing same-process producers/consumers. |
