...
Evidence snapshot: `2026-09-05T14:30:57+08:00` (`2026-09-05T06:30:57Z`)
...
The 15 move packets are structurally complete, but the release control plane is not safe for an
all-at-once release:
...
- [Observed] Scoped `bd lint` over all 15 move IDs, including closed `.3`, returns zero findings.
  Every move has non-empty description, design, and acceptance-criteria fields.
...
- **Packet complete** means the move has non-empty description, design, and acceptance criteria and
  passes scoped `bd lint`. It does not mean its specs are signed off or that it can run now.
...
|---|---|---|---|---|---|---|
| 1 / `bu-7exe4.1` | Open; unassigned; no heartbeat | `bu-xf54r` | No matching PR, branch, or worktree | Yes | No | **HOLD.** First serialize before `.9` and other core migration work; then a separate explicit close of `bu-xf54r` may release it. |
| 2 / `bu-7exe4.2` | Blocked; foreign assignee `coordinator:321c...`; heartbeat `2026-09-02T19:13:24Z`, stale | `bu-xf54r`; `bu-psarp` | Draft/conflicting [PR #3960](https://github.com/tzeusy-org/butlers/pull/3960), head `cf0ac58`; clean exact remote worktree `bu-lp4bq` | Yes | No | **HOLD and preserve.** Gate closure is insufficient. A separate owner-authorized production rollout/restart must satisfy `bu-psarp`; the foreign lane must then rebase/renumber and regain exact-head review/CI. Do not reclaim or edit it here. |
| 3 / `bu-7exe4.3` | Closed; historical assignee only; last heartbeat `2026-09-02T17:56:33Z`, not a live claim | Closed review bead `bu-69ufc`; gate remains a historical dependency | [PR #3959](https://github.com/tzeusy-org/butlers/pull/3959) merged at head `11658a85`, merge commit `b5e08ad3` | Yes | N/A | **COMPLETE.** No owner release act remains and the gate must not reopen this work. |
| 4 / `bu-7exe4.4` | Open; unassigned; no heartbeat | `bu-xf54r` | No matching PR, branch, or worktree | Yes | No | **RELEASE CANDIDATE after control-plane repair.** The future owner act is an explicit close of `bu-xf54r`; it still does not authorize destructive webhook operations outside the implemented confirmation contract. |
| 5 / `bu-7exe4.5` | Open; unassigned; no heartbeat | `bu-xf54r` | No matching PR, branch, or worktree | Yes | No | **RELEASE CANDIDATE after control-plane repair.** Future owner act: explicitly close `bu-xf54r`. |
| 6 / `bu-7exe4.6` | Open; unassigned; no heartbeat | `bu-xf54r`; `.15` | No matching PR, branch, or worktree | Yes | No | **HOLD.** `.15` must land first, and the `core-notify` / notification-gate surface must serialize with `.13` specification work. Gate closure alone will not make this runnable because `.15` is encoded as a blocker. |
| 7 / `bu-7exe4.7` | Open; unassigned; no heartbeat | `bu-xf54r` | No matching move PR. Foreign `bu-8cdl1.8` is assigned to another coordinator and its worktree has 15 uncommitted paths after merged PR #4005 | Yes | No | **HOLD and preserve unpublished work.** Resolve/land or explicitly park the foreign entity-graph slice, then encode serialization and refresh before any owner closes `bu-xf54r`. |
| 8 / `bu-7exe4.8` | Open; unassigned; no heartbeat | `bu-xf54r` | Foreign [PR #4015](https://github.com/tzeusy-org/butlers/pull/4015) is open but `CONFLICTING`/`DIRTY`; its successful rollup is stale-base evidence. Clean worktree/remote head `e90f6080` edits `RootLayout.tsx` | Yes | No | **HOLD.** Let the foreign RootLayout lane settle, refresh `main`, then release only after the overlap is gone or serialized. |
| 9 / `bu-7exe4.9` | Open; unassigned; no heartbeat | `bu-xf54r` | No matching PR, branch, or worktree | Yes | No | **HOLD.** Serialize after higher-priority `.1`; pin the exact model-resolution spec before implementation and coordinate core migration numbering with live PRs. |
| 10 / `bu-7exe4.10` | Open; unassigned; no heartbeat | `bu-xf54r` | No matching move artifact. Foreign PRs #4012 and #4015 are both open but `CONFLICTING`/`DIRTY`; they edit `index.css` and `RootLayout.tsx` respectively. PR #4012's `check-integration-5` and fan-in `check` failed; PR #4015's rollup passed on its stale base | Yes | No | **HOLD, spec first.** The packet says design-language doctrine *or* the dashboard design spec, while the live dashboard-shell spec still mandates a system font stack. Produce and separately approve one exact normative delta before implementation; settle foreign layout/CSS work first. |
| 11 / `bu-7exe4.11` | Open; unassigned; no heartbeat | `bu-xf54r`; `.10` | No matching PR, branch, or worktree | Yes | No | **HOLD.** `.10` is an encoded prerequisite; land its exact spec/first-frame contract before focus-token implementation. |
| 12 / `bu-7exe4.12` | Open; unassigned; no heartbeat | `bu-xf54r` | No matching move artifact. `bu-ip5c5` is a stale foreign claim on `EntityFinder`, with no PR/worktree found | Yes | No | **HOLD.** An authorized recovery/ownership pass must resolve `bu-ip5c5`, and the shortcut/palette requirements need an exact `dashboard-shell` delta before implementation. Then refresh before gate release. |
| 13 / `bu-7exe4.13` | Open epic; unassigned; no heartbeat | `bu-xf54r`; nested `.13.1` through `.13.5` sequencing | No PR, branch, worktree, or live voice-egress OpenSpec change found | Yes | No | **SPEC-ONLY RELEASE CANDIDATE; IMPLEMENTATION HOLD.** Closing `bu-xf54r` may authorize `.13.1` to draft the contract. The owner must separately approve that exact artifact. Provider/device/presence/audio/credential/runtime work requires another explicit act and remains forbidden now. |
| 14 / `bu-7exe4.14` | Open; unassigned; no heartbeat | `bu-xf54r`; `.2` | No matching move artifact. Foreign [PR #4012](https://github.com/tzeusy-org/butlers/pull/4012) is `CONFLICTING`/`DIRTY` with failed `check-integration-5` and fan-in `check`; clean worktree/remote head `5c8531ee` edits `MessageThread.tsx`, conversation API/data, migration, and the governing spec | Yes | No | **HOLD.** `.2` must complete and PR #4012 must settle before this lane is shaped against refreshed conversation contracts. Gate closure alone cannot make it runnable because `.2` is encoded as a blocker. |
| 15 / `bu-7exe4.15` | Open; unassigned; no heartbeat | `bu-xf54r` | No matching PR, branch, or worktree | Yes | No | **RELEASE CANDIDATE after control-plane repair.** Run before `.6`, as already encoded. Future owner act: explicitly close `bu-xf54r`; no live messaging or runtime action follows from that act. |
...
| 9 / `.9` | [routing-scorecard](../../openspec/specs/routing-scorecard/spec.md) is present, but the packet still says “model-routing (or routing-scorecard)” and therefore has not pinned one exact requirement | Exact implementation/storage overlap with `.1`; migration sequence also shares the live global core chain. |
...
| 14 / `.14` | [butler-switchboard](../../openspec/specs/butler-switchboard/spec.md), [butler-messenger](../../openspec/specs/butler-messenger/spec.md), [core-notify](../../openspec/specs/core-notify/spec.md), [core-spawner](../../openspec/specs/core-spawner/spec.md), [dashboard-conversations](../../openspec/specs/dashboard-conversations/spec.md); planned deltas required | Exact overlap with `.2`; exact conversation API/UI/spec overlap with foreign PR #4012. |
...
2. [Inferred] Serialize `.1` before `.9`. Both own attempt/resolution persistence, spawner/model
   routing, and Spend/Models surfaces; `.1` is higher priority.
...
4. [Observed] Hold `.8` until PR #4015 settles; hold `.10` until PRs #4012/#4015 settle and its font
   contract is approved; hold `.14` until both `.2` and PR #4012 settle.
...
7. [Observed] Any move adding a core migration must refresh the global revision chain immediately
   before publishing. Open PRs #4012 and #4016 currently add core revisions.
...
| Independent report review | Pass after correction of gate-readiness wording, live PR health, exact spec headings, and `.3` heartbeat evidence; no factual or authority-boundary findings remained |
...
- “Packet complete” would be false if inferred from `bd lint` alone. This report also checked each
  move's description, design, and acceptance fields for non-empty content, then separately assessed
