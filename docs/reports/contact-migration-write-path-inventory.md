...
1. **`secured=true` carve-out:** `roster/relationship/api/router.py:1919` is the only write path that can insert `secured=true` rows into `public.contact_info`. Per RFC 0004 Amendment 2 §"public.entity_info", credential rows (`secured=true`) are **out of scope** for the triples migration and will move to `public.entity_info` instead of `relationship.entity_facts`. The re-pointing plan stub for that row reflects this.
