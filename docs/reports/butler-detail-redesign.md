...
| Panel renders title eyebrow | bu-iuol4.13 | `atoms.test.tsx:173+` (Panel title and subsection) | pass |
...
| Each butler renders its registered bespoke tab label | per-bespoke beads | per-tab test files + `ButlerDetailPage.test.tsx` | pass (11/11; general shipped in PR #1581) |
...
| New butlers (general, lifestyle, messenger, qa) include bespoke tabs | bu-iuol4.28/.30/.33/.34 | `ButlerLifestyleTasteTab.test.tsx`, `ButlerMessengerConversationsTab.test.tsx`, `ButlerQaInvestigationsTab.test.tsx`, `ButlerGeneralCollectionsTab.test.tsx` | pass (4/4) |
...
2. **Prose copy violation (now fixed):** `ButlerEducationReviewsTab.tsx:239` previously rendered `"No reviews scheduled — keep learning and reviews will appear here."` and line 361 rendered `"No frontier nodes yet — keep mastering prerequisites!"`. Both used em-dash as a prose separator.

**Status: PASS.** PR #1582 (bu-iuol4.26) merged and fixed the em-dash violations in `ButlerEducationReviewsTab.tsx`. All butler-detail TSX files are now clean.
...
- education → reviews ✓ (redesign shipped in PR #1582)
- finance → finances ✓
- health → health ✓ (registered as "health" tab value, rendered as "Health" label in `ButlerDetailPage.tsx`)
- home → devices ✓
- lifestyle → taste ✓
...
- travel → trips ✓
- general → collections ✓ (shipped in PR #1581; `GENERAL_TABS` constant added to `butler-detail-tabs.ts`)
- switchboard → no bespoke tab ✓ (only routing-log and registry)

**Status: PASS.** All 11 domain butlers have bespoke tabs registered.
...
| education | RESTYLED | Reviews | bu-iuol4.26 | #1582 |
| finance | RESTYLED | Finances | bu-iuol4.29 | #1549 |
| general | NEW | Collections | bu-iuol4.30 | #1581 |
| health | RESTYLED | Health | bu-iuol4.23 | #1551 |
...
| switchboard | SKIP | (operator-only: Routing Log + Registry) | n/a | n/a |
| travel | RESTYLED | Trips | bu-iuol4.36 | #1558 |

**Notes:**
- Education: `ButlerEducationReviewsTab.tsx` existed before this epic. bu-iuol4.26 (PR #1582) restyled it to the 4-col Panel grid and fixed the em-dash violations in empty-state prose.
- General: `ButlerGeneralCollectionsTab` shipped in PR #1581 (bu-iuol4.30). The `GENERAL_TABS` constant was added to `butler-detail-tabs.ts` and the tab is now registered. bu-iuol4.31 (backend stats endpoint) merged as PR #1577.
- Health: The tab value is `"health"` in the tabs registry (from the original HEALTH_TABS constant) and is displayed as "Health" in the tab trigger in ButlerDetailPage. The full rewrite (bu-iuol4.23) delivered a proper 4-col Panel grid replacing the 6-link directory.
...
**Recommendation:** Both changes should be archived now that all blocked beads (bu-iuol4.26 and bu-iuol4.30) have shipped. Archiving is safe once the three remaining deferred resident-tab beads (bu-iuol4.16, bu-iuol4.19, bu-iuol4.20) are acknowledged as separate follow-up work not gated on these specs.
...
**Pattern observed across 10 of the 10 implemented bespoke tabs:**
...
- **Prohibited:** Free text like `"No reviews scheduled — keep learning and reviews will appear here."` (the original `ButlerEducationReviewsTab.tsx:239` before PR #1582). This is an em-dash used as a prose connective.
...
### 7.5 `general` butler bespoke tab shipped

The general butler's Collections tab (bu-iuol4.30) shipped in PR #1581. The `GENERAL_TABS` constant is now in `butler-detail-tabs.ts`. For reference, the pattern used:
- `const GENERAL_TABS = ["collections"] as const;` added to `butler-detail-tabs.ts`
- Wired in `getAllTabs` with `if (butlerName === "general") { baseTabs.push(...GENERAL_TABS); }`
- Lazy import and render added in `ButlerDetailPage.tsx`
