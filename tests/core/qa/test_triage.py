...
def _patch_triage(insert_side=None, active_rv=None, dismissed_rv=False, recent_rv=None):
    ...
    return (
        patch(
            ...
            side_effect=insert_side or AsyncMock(return_value=uuid.uuid4()),
        ),
        patch(
            "butlers.core.qa.triage.get_active_attempt",
            new_callable=AsyncMock,
            return_value=active_rv,
        ),
        patch(
            "butlers.core.qa.triage.is_dismissed", new_callable=AsyncMock, return_value=dismissed_rv
        ),
        patch(
            "butlers.core.qa.triage.get_recent_attempt",
            new_callable=AsyncMock,
            return_value=recent_rv,
        ),
    )
...
async def test_triage_is_novel_empty_and_novel():
    ...
    assert (
        TriagedFinding(finding=finding, dedup_reason=None, finding_id=uuid.uuid4()).is_novel is True
    )
    assert (
        TriagedFinding(
    ...
    # Empty
    with (
        patch("butlers.core.qa.triage.insert_finding", new_callable=AsyncMock) as mock_ins,
    ...
    with (
        patch(
            "butlers.core.qa.triage.insert_finding", new_callable=AsyncMock, return_value=finding_id
        ),
        patch(
    ...
    assert (
        result.novel_findings[0].is_novel is True
        and result.novel_findings[0].finding_id == finding_id
    )
...
async def test_triage_intra_patrol_dedup_and_sorting():
    ...
    with (
        patch("butlers.core.qa.triage.insert_finding", side_effect=insert_side),
        patch(
            "butlers.core.qa.triage.get_active_attempt", new_callable=AsyncMock, return_value=None
        ),
        patch("butlers.core.qa.triage.is_dismissed", new_callable=AsyncMock, return_value=False),
    ...
    with (
        patch("butlers.core.qa.triage.insert_finding", side_effect=insert_seq),
        patch(
            "butlers.core.qa.triage.get_active_attempt", new_callable=AsyncMock, return_value=None
        ),
        patch("butlers.core.qa.triage.is_dismissed", new_callable=AsyncMock, return_value=False),
