...
@pytest.mark.asyncio
async def test_save_cursor_keeps_canonical_registry_row_active_by_default() -> None:
    pool = _FakePool()

    await save_cursor(
        pool,  # type: ignore[arg-type]
        "gmail",
        "gmail:user:account@example.invalid",
        "cursor-value",
    )

    assert pool.connection.execute_args is not None
    sql, *_args = pool.connection.execute_args
