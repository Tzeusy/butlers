...
async def test_small_group_bypasses_discretion() -> None:
    ...
    result = await evaluator.evaluate(text="lol nice one", weight=0.1, participant_count=8)
...
async def test_large_group_runs_full_discretion() -> None:
    ...
    result = await evaluator.evaluate(text="ambient chatter", weight=0.7, participant_count=300)
...
async def test_boundary_at_threshold_bypasses() -> None:
    ...
    result = await evaluator.evaluate(text="banter", weight=0.1, participant_count=20)
