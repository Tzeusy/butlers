...
def _make_action(*, action_id=None, tool_name="telegram_send_message", status="pending"):
    return {
        "id": action_id or _ACTION_ID, "tool_name": tool_name,
        "tool_args": {"chat_id": "12345", "text": "Hello"}, "status": status,
        "requested_at": _NOW, "agent_summary": None, "session_id": None,
        "expires_at": None, "decided_by": None, "decided_at": None,
        "execution_result": None, "approval_rule_id": None,
    }


def _make_rule(*, rule_id=None, tool_name="telegram_send_message"):
    return {
        "id": rule_id or _RULE_ID, "tool_name": tool_name,
        "arg_constraints": {"chat_id": {"type": "exact", "value": "12345"}},
        "description": "Auto-approve messages", "created_from": None,
        "created_at": _NOW, "expires_at": None, "max_uses": None,
        "use_count": 0, "active": True,
    }


def _app_with_mock_db(
    app, *, has_approvals_tables=True, fetch_rows=None,
    fetchval_return=None, fetchrow_return=None,
):
    mock_conn = AsyncMock()
...
async def test_create_rule_invalid_max_uses_returns_422(app):
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app),
                                 base_url="http://test") as client:
        resp = await client.post("/api/approvals/rules", json={
            "tool_name": "x", "max_uses": -1,
        })
