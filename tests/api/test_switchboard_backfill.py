...
class TestListBackfillJobs:
    ...
    async def test_returns_job_summaries(self):
        ...
        app = _app_with_mock_db(fetchval_result=1, fetch_rows=[_SAMPLE_JOB_ROW])
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.get("/api/switchboard/backfill")
    ...
    async def test_filter_by_connector_type_accepted(self):
    ...
    async def test_filter_by_invalid_status_returns_422(self):
    ...
    async def test_degraded_db_returns_empty_list(self):
    ...
    async def test_active_job_in_list(self):
        """Active jobs appear in list with their status and counters."""
        app = _app_with_mock_db(fetchval_result=1, fetch_rows=[_ACTIVE_JOB_ROW])
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.get("/api/switchboard/backfill")
...
class TestCreateBackfillJob:
    ...
    async def test_creates_job_returns_201(self):
        """Successful creation returns 201 with created job data."""
        app = _app_with_mock_db(fetchrow_result=_SAMPLE_JOB_ROW)
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
    ...
    async def test_created_job_has_id(self):
    ...
    async def test_db_error_on_insert_returns_503(self):
        """When INSERT fails, returns 503."""
        app = _app_with_mock_db(fetchrow_side_effect=Exception("DB write failed"))
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.post("/api/switchboard/backfill", json=self._CREATE_PAYLOAD)
    ...
    async def test_default_rate_limits_applied(self):
...
class TestGetBackfillJob:
    ...
    async def test_db_error_returns_503(self):
        ...
        app = _app_with_mock_db(fetchrow_side_effect=Exception("DB error"))
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.get(f"/api/switchboard/backfill/{_JOB_ID}")
...
class TestPauseBackfillJob:
    async def test_pauses_active_job(self):
        ...
        app = _app_with_mock_db(fetchrow_side_effects=[{"status": "active"}, None])
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
    ...
    async def test_pauses_pending_job(self):
        """Pending job can also be paused."""
        app = _app_with_mock_db(fetchrow_side_effects=[{"status": "pending"}, None])
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.patch(f"/api/switchboard/backfill/{_JOB_ID}/pause")
    ...
    async def test_already_paused_returns_409(self):
        """Pausing an already-paused job returns 409 conflict."""
        app = _app_with_mock_db(fetchrow_result={"status": "paused"})
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.patch(f"/api/switchboard/backfill/{_JOB_ID}/pause")
    ...
    async def test_completed_job_returns_409(self):
        ...
        app = _app_with_mock_db(fetchrow_result={"status": "completed"})
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.patch(f"/api/switchboard/backfill/{_JOB_ID}/pause")
    ...
    async def test_db_error_on_fetch_returns_503(self):
...
class TestCancelBackfillJob:
    ...
    async def test_cancels_paused_job(self):
        """Paused job can be cancelled."""
        app = _app_with_mock_db(fetchrow_side_effects=[{"status": "paused"}, None])
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.patch(f"/api/switchboard/backfill/{_JOB_ID}/cancel")
    ...
    async def test_already_cancelled_job_returns_409(self):
        """Cancelling an already-cancelled job returns 409."""
        app = _app_with_mock_db(fetchrow_result={"status": "cancelled"})
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.patch(f"/api/switchboard/backfill/{_JOB_ID}/cancel")
...
class TestResumeBackfillJob:
    ...
    async def test_active_job_cannot_be_resumed(self):
        """Resuming an active (not paused) job returns 409."""
        app = _app_with_mock_db(fetchrow_result={"status": "active"})
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.patch(f"/api/switchboard/backfill/{_JOB_ID}/resume")
    ...
    async def test_completed_job_cannot_be_resumed(self):
    ...
    async def test_cancelled_job_cannot_be_resumed(self):
...
class TestGetBackfillJobProgress:
    async def test_returns_progress_metrics(self):
        """Progress endpoint returns rows_processed, rows_skipped, cost_spent_cents."""
        app = _app_with_mock_db(fetchrow_result=_ACTIVE_JOB_ROW)
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
    ...
    async def test_completed_job_progress(self):
        ...
        app = _app_with_mock_db(fetchrow_result=completed_row)
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.get(f"/api/switchboard/backfill/{_JOB_ID}/progress")
