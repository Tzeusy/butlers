...
# ---------------------------------------------------------------------------
# GET /api/costs/daily — ?butler= filter [bu-lryu6]
# ---------------------------------------------------------------------------
...
async def test_daily_no_butler_filter_aggregates_all(app):
    ...
    async with httpx.AsyncClient(
        ...
        resp = await client.get(
            "/api/costs/daily", params={"from": "2026-05-02", "to": "2026-05-02"}
        )
    assert resp.status_code == 200
    data = resp.json()["data"]
...
# ---------------------------------------------------------------------------
# GET /api/costs/top-sessions — ?butler= filter [bu-lryu6]
# ---------------------------------------------------------------------------
...
async def test_top_sessions_butler_filter_returns_only_that_butler(app):
    ...
    async with httpx.AsyncClient(
        ...
        resp = await client.get("/api/costs/top-sessions", params={"butler": "sw"})
    assert resp.status_code == 200
    data = resp.json()["data"]
...
async def test_top_sessions_no_butler_filter_aggregates_all(app):
    ...
    async with httpx.AsyncClient(
        ...
        resp = await client.get("/api/costs/top-sessions")
    assert resp.status_code == 200
    data = resp.json()["data"]
...
async def test_top_sessions_unknown_butler_returns_empty_200(app):
    ...
    async with httpx.AsyncClient(
        ...
        resp = await client.get("/api/costs/top-sessions", params={"butler": "nonexistent"})
    assert resp.status_code == 200
    assert resp.json()["data"] == []


# ---------------------------------------------------------------------------
# GET /api/costs/by-schedule — ?butler= filter [bu-lryu6]
# ---------------------------------------------------------------------------
...
async def test_by_schedule_butler_filter_returns_only_that_butler(app):
    ...
    async with httpx.AsyncClient(
        ...
        resp = await client.get("/api/costs/by-schedule", params={"butler": "sw"})
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert all(s["butler"] == "sw" for s in data)
...
async def test_by_schedule_unknown_butler_returns_empty_200(app):
    ...
    async with httpx.AsyncClient(
        ...
        resp = await client.get("/api/costs/by-schedule", params={"butler": "nonexistent"})
    assert resp.status_code == 200
    assert resp.json()["data"] == []
