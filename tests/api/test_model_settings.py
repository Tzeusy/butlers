...
async def test_verify_all_endpoint_passes_system_global_codex_authority(app, monkeypatch):
    """Manual model verification derives Codex authority from the shared credential pool."""
    import butlers.api.routers.model_settings as model_settings

    monkeypatch.setattr(model_settings, "_verify_all_last_run", 0.0)
    _, pool = _app_with_pool(app)
    authority = MagicMock()
    store_cls = MagicMock(return_value=authority)
    monkeypatch.setattr(model_settings, "CredentialStore", store_cls, raising=False)
    verify = AsyncMock(
        return_value=model_settings.VerifyAllResult(accepted=True, total=0, ok=0, failed=0)
    )
    monkeypatch.setattr(model_settings, "run_verify_all_models", verify)
...
async def test_run_verify_all_marks_partial_failure_as_audit_error(monkeypatch):
    ...
    result = await model_settings.run_verify_all_models(
        pool,
...
async def test_run_verify_all_injects_authority_only_for_codex(monkeypatch):
    """Codex verification gets the protected store; other runtimes keep their factory shape."""
    import butlers.api.routers.model_settings as model_settings

    pool = AsyncMock()
    pool.fetch = AsyncMock(
        return_value=[
            {"id": uuid.uuid4(), "runtime_type": "codex", "model_id": "gpt-5", "extra_args": []},
            {
                "id": uuid.uuid4(),
                "runtime_type": "claude",
    ...
    def adapter_cls_for(runtime_type: str):
        class _Adapter:
            def __init__(self, **kwargs) -> None:
                constructor_kwargs[runtime_type] = kwargs
    ...
    monkeypatch.setattr(model_settings, "get_adapter", adapter_cls_for)
    monkeypatch.setattr(model_settings, "resolve_provider_config", AsyncMock(return_value={}))
    monkeypatch.setattr(model_settings.audit, "append", AsyncMock(return_value=1))
    authority = MagicMock()
    ...
    assert result.ok == 2
    assert constructor_kwargs["codex"] == {"credential_store": authority}
    assert constructor_kwargs["claude"] == {"provider_config": {}}


async def test_run_verify_all_skips_codex_without_authority_without_persisting(monkeypatch):
    """An unavailable authority cannot poison Codex routing eligibility as a failed probe."""
    import butlers.api.routers.model_settings as model_settings
    ...
    pool.fetch = AsyncMock(
        return_value=[
            {"id": entry_id, "runtime_type": "codex", "model_id": "gpt-5", "extra_args": []}
        ]
    )
    pool.execute = AsyncMock(return_value="UPDATE 1")
    get_adapter = MagicMock()
    audit_append = AsyncMock(return_value=1)
    monkeypatch.setattr(model_settings, "get_adapter", get_adapter)
    monkeypatch.setattr(model_settings.audit, "append", audit_append)

    result = await model_settings.run_verify_all_models(pool)

    assert (result.total, result.ok, result.failed, result.skipped) == (1, 0, 0, 1)
    get_adapter.assert_not_called()
    pool.execute.assert_not_awaited()
    assert audit_append.await_args.kwargs["result"] == "success"
    assert "skipped" in audit_append.await_args.kwargs["note"]


async def test_run_verify_all_skips_codex_without_explicit_system_global_authority(monkeypatch):
    """A generic store is not an authority substitute for a Codex verification probe."""
    import butlers.api.routers.model_settings as model_settings
    ...
    result = await model_settings.run_verify_all_models(
        pool,
        codex_auth_authority=CredentialStore(MagicMock()),
    )

    assert (result.ok, result.failed, result.skipped) == (0, 0, 1)
    pool.execute.assert_not_awaited()


async def test_catalog_entry_test_passes_system_global_codex_authority(app, monkeypatch):
    """The per-entry dashboard test does not instantiate Codex without its authority."""
    import butlers.api.routers.model_settings as model_settings
    ...
    monkeypatch.setattr(model_settings, "get_adapter", lambda _runtime_type: _CodexAdapter)
    monkeypatch.setattr(model_settings, "resolve_provider_config", AsyncMock(return_value={}))

    async with httpx.AsyncClient(
        transport=httpx.ASGITransport(app=app), base_url="http://test"
    ) as client:
        response = await client.post(f"/api/settings/models/{entry_id}/test")

    assert response.status_code == 200
    assert response.json()["data"]["success"] is True
    store_cls.assert_called_once_with(pool, system_global_pool=pool)
    assert constructor_kwargs == {"credential_store": authority}


async def test_catalog_entry_test_redacts_codex_provider_diagnostic(app, monkeypatch, caplog):
    """Codex probe diagnostics stay out of both the API response and logs."""
    import butlers.api.routers.model_settings as model_settings
    ...
    class _CodexAdapter:
        ...
        async def invoke(self, **_kwargs):
            raise RuntimeError("provider-diagnostic-secret-marker")

    monkeypatch.setattr(model_settings, "get_adapter", lambda _runtime_type: _CodexAdapter)
    monkeypatch.setattr(model_settings, "resolve_provider_config", AsyncMock(return_value={}))

    with caplog.at_level(logging.WARNING, logger=model_settings.__name__):
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            response = await client.post(f"/api/settings/models/{entry_id}/test")
    ...
    assert response.status_code == 200
    assert response.json()["data"]["error"] == "Codex model verification failed."
    assert "provider-diagnostic-secret-marker" not in response.text
    assert "provider-diagnostic-secret-marker" not in caplog.text
