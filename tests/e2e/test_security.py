...
1. Env isolation: Undeclared env vars are not accessible to runtime instances
2. Cross-DB isolation: Butler tools cannot access other butler databases
3. MCP config scope: Each runtime instance only sees its own butler's tools
...
async def test_env_var_isolation_canary_not_accessible(
    ...
    """runtime instance should not access undeclared environment variables.

    Sets a canary env var, triggers health butler, verifies the runtime instance
    ...
    assert canary_value not in result.output, (
        "Canary env var leaked to runtime instance! "
...
async def test_mcp_config_lockdown_health_tools_only(
    ...
    """Health butler's runtime instance should only see health tools.
    ...
    assert not leaked_switchboard_tools, (
        f"Health butler runtime instance should not see switchboard tools, "
