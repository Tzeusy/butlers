...
class TestMigrationFileAndChain:
    ...
    def test_chain_head_resolves_to_core_184(self) -> None:
        """The real migration-chain scan (used by the migration-drift sentinel
        and CI's migration-integrity gate) must see core_184 as the current
        head -- proves core_183 chained onto the true head rather than a
        stale/guessed revision number, and that core_184 (bu-ep4ks.6,
        owner_conditions) has since chained cleanly onto core_183 with no
        gap. Update this literal again the next time a core migration lands."""
        from butlers.migrations import get_chain_head

        assert get_chain_head("core") == "core_184"
