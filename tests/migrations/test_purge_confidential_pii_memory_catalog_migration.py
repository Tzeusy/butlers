...
class TestMigrationFileAndChain:
    ...
    def test_chain_head_resolves_to_core_185(self) -> None:
        """The real migration-chain scan (used by the migration-drift sentinel
        and CI's migration-integrity gate) must see core_185 as the current
        head -- proves core_183 chained onto the true head rather than a
        stale/guessed revision number, and that core_184 (bu-ep4ks.6,
        owner_conditions) and core_185 (bu-ep4ks.8, conversation anchor +
        provider resume ledger) have since chained cleanly on top of it with
        no gap. Update this literal again the next time a core migration
        lands."""
        from butlers.migrations import get_chain_head

        assert get_chain_head("core") == "core_185"
