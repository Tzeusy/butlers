...
Covers tasks 2.1–2.3 from openspec/changes/dashboard-conversational-input/tasks.md:
  ...
  2.1  public.dashboard_conversations table with all required columns.
  2.2  public.dashboard_messages table with all required columns.
...
# ---------------------------------------------------------------------------
# Task 2.1 — public.dashboard_conversations table
# ---------------------------------------------------------------------------
...
class TestDashboardConversationsTable:
    ...
    def test_creates_dashboard_conversations_table(self) -> None:
        """upgrade() creates public.dashboard_conversations."""
        mod = _load_migration()
        source = inspect.getsource(mod.upgrade)
        assert "CREATE TABLE IF NOT EXISTS public.dashboard_conversations" in source
...
# ---------------------------------------------------------------------------
# Task 2.2 — public.dashboard_messages table
# ---------------------------------------------------------------------------


class TestDashboardMessagesTable:
    def test_creates_dashboard_messages_table(self) -> None:
        """upgrade() creates public.dashboard_messages."""
        mod = _load_migration()
        source = inspect.getsource(mod.upgrade)
        assert "CREATE TABLE IF NOT EXISTS public.dashboard_messages" in source

    def test_messages_fk_to_conversations(self) -> None:
        """dashboard_messages.conversation_id has FK to dashboard_conversations."""
        mod = _load_migration()
        source = inspect.getsource(mod.upgrade)
        assert "REFERENCES public.dashboard_conversations(id)" in source
...
class TestDowngrade:
    def test_drops_dashboard_messages(self) -> None:
        """downgrade() drops public.dashboard_messages."""
        mod = _load_migration()
        source = inspect.getsource(mod.downgrade)
        assert "DROP TABLE IF EXISTS public.dashboard_messages" in source

    def test_drops_dashboard_conversations(self) -> None:
        """downgrade() drops public.dashboard_conversations."""
        mod = _load_migration()
        source = inspect.getsource(mod.downgrade)
        assert "DROP TABLE IF EXISTS public.dashboard_conversations" in source
