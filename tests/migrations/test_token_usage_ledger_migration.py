"""Tests for core_035: public.token_usage_ledger and public.token_limits migration."""
...
class TestCore035LedgerTableDefinition:
    ...
    def test_creates_token_usage_ledger(self) -> None:
        assert "public.token_usage_ledger" in self._src()

    def test_create_if_not_exists(self) -> None:
        assert "CREATE TABLE IF NOT EXISTS public.token_usage_ledger" in self._src()
    ...
    def test_catalog_entry_id_references_model_catalog_cascade(self) -> None:
        src = self._src()
        assert "public.model_catalog(id) ON DELETE CASCADE" in src
...
class TestCore035LedgerIndex:
    ...
    def test_idx_ledger_entry_time_on_catalog_entry_id_recorded_at(self) -> None:
        src = self._src()
        assert "ON public.token_usage_ledger (catalog_entry_id, recorded_at)" in src
...
class TestCore035LimitsTableDefinition:
    ...
    def test_creates_token_limits(self) -> None:
        assert "public.token_limits" in self._src()

    def test_create_if_not_exists(self) -> None:
        assert "CREATE TABLE IF NOT EXISTS public.token_limits" in self._src()
    ...
    def test_catalog_entry_id_fk_cascade(self) -> None:
        ...
        # public.model_catalog(id) ON DELETE CASCADE; == 2 catches a regression
        # where one table loses the constraint.
        assert src.count("public.model_catalog(id) ON DELETE CASCADE") == 2
...
class TestCore035Downgrade:
    ...
    def test_drops_token_limits(self) -> None:
        assert "DROP TABLE IF EXISTS public.token_limits" in self._src()
    ...
    def test_drops_index_before_tables(self) -> None:
        ...
        table_pos = src.find("DROP TABLE IF EXISTS public.token_limits")
    ...
    def test_drops_token_limits_before_ledger(self) -> None:
        ...
        src = self._src()
        limits_pos = src.find("DROP TABLE IF EXISTS public.token_limits")
        ledger_pos = src.find("DROP TABLE IF EXISTS public.token_usage_ledger CASCADE")
