...
class TestSeedOwnerTelegramHandle:
    ...
    async def test_seeds_prefixed_handle_from_chat_id(self) -> None:
        ...
        conn = AsyncMock()
        # 1. schema check -> relationship; 2. tables_ready check -> True;
        # 3. chat_id lookup -> "206570151"
        conn.fetchval = AsyncMock(side_effect=["relationship", True, "206570151"])
    ...
    async def test_no_chat_id_is_noop(self) -> None:
        ...
        conn = AsyncMock()
        conn.fetchval = AsyncMock(side_effect=["relationship", True, None])
        conn.execute = AsyncMock()
        await _seed_owner_telegram_handle(conn, _OWNER_ENTITY_ID)
        conn.execute.assert_not_awaited()
    ...
    async def test_non_relationship_schema_noops_without_warning(self) -> None:
        from butlers.owner_bootstrap import _seed_owner_telegram_handle
