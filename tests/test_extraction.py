...
class TestExtractSignals:
    ...
    async def test_high_confidence_dispatched(self, pool):
        ...
        await register_butler(pool, "relationship", "http://localhost:8101/sse")
...
# ------------------------------------------------------------------
# Concurrent execution (handle_message_with_extraction)
# ------------------------------------------------------------------


class TestHandleMessageWithExtraction:
    """Tests for concurrent classification + extraction."""

    async def test_concurrent_classify_and_extract(self, pool):
        """Both classification and extraction run and return."""
        from butlers.tools.extraction import (
            handle_message_with_extraction,
        )
        from butlers.tools.switchboard import register_butler
        ...
        async def classify_dispatch(**kwargs):
            return FakeSpawnerResult(result="relationship")

        extract_response = _single_extraction(tool_args={"name": "Iris"})

        async def extract_dispatch(**kwargs):
            return FakeSpawnerResult(result=extract_response)

        result = await handle_message_with_extraction(
            pool,
            "I met Iris today, she's lovely",
            classify_dispatch,
            extract_dispatch,
            route_call_fn=_mock_call,
        )

        assert result["target_butler"] == "relationship"
        assert len(result["extractions"]) == 1
        assert result["extractions"][0].target_butler == "relationship"

    async def test_extraction_failure_no_block(self, pool):
        """If extraction fails, classification still succeeds."""
        from butlers.tools.extraction import (
            handle_message_with_extraction,
        )
        from butlers.tools.switchboard import register_butler
        ...
        async def classify_dispatch(**kwargs):
            return FakeSpawnerResult(result="general")
    ...
    async def test_classification_failure_defaults_general(self, pool):
        """If classification fails, defaults to 'general'."""
        from butlers.tools.extraction import (
            handle_message_with_extraction,
        )
        from butlers.tools.switchboard import register_butler
        ...
        async def classify_dispatch(**kwargs):
            raise RuntimeError("Classify CC crashed")
    ...
    async def test_both_fail_gracefully(self, pool):
        """If both fail, still returns gracefully."""
        from butlers.tools.extraction import (
            handle_message_with_extraction,
        )
        from butlers.tools.switchboard import register_butler
...
class TestCustomSchemas:
    ...
    async def test_custom_schema_single_butler(self, pool):
        ...
        await pool.execute("DELETE FROM butler_registry")
        await pool.execute("DELETE FROM routing_log")
        await register_butler(pool, "finance", "http://localhost:8103/sse")
