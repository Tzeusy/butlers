...
from butlers.modules.google_drive import (
    GoogleDriveConfig,
    GoogleDriveModule,
    GoogleDriveStartupError,  # noqa: F401 — exported sentinel; tested for presence in TestModuleABCCompliance
    _DriveHTTPClient,
    _infer_mime_type,
)
...
class TestOnStartup:
    ...
    async def test_startup_without_credential_store_stays_degraded(self):
        """No credential_store -> module stays in degraded state (logs warning, no exception)."""
        mod = GoogleDriveModule()
        # Should not raise — stays degraded
    ...
    async def test_startup_missing_credentials_stays_degraded(self):
        """MissingGoogleCredentialsError -> module stays degraded (logs warning, no exception)."""
    ...
    async def test_startup_scope_validation_fails_for_readonly_only(self):
        """drive.readonly scope -> module stays degraded (logs warning, no exception)."""
        mod = GoogleDriveModule()
        creds = MagicMock()
    ...
    async def test_startup_account_not_found_stays_degraded(self):
        """Account not in DB -> module stays degraded (logs warning, no exception)."""
        from butlers.google_credentials import MissingGoogleCredentialsError
    ...
    async def test_shutdown_closes_http_client_without_wrapper(self):
        """on_shutdown closes _http_client directly when _client is None (degraded path)."""
        mod = GoogleDriveModule()
        mock_http = MagicMock()
...
class TestDriveGetFileMetadata:
    ...
    async def test_no_credentials_returns_error(self):
        ...
        result = await module._drive_get_file_metadata(file_id="any")
...
class TestDriveReadFile:
    ...
    async def test_no_credentials_returns_error(self):
        ...
        result = await module._drive_read_file(file_id="any")
...
class TestDriveWriteFile:
    ...
    async def test_no_credentials_returns_error(self):
        ...
        result = await module._drive_write_file(
            name="test.txt", content="data", folder_id=None, mime_type=None
        )
...
class TestDriveMoveFile:
    ...
    async def test_no_credentials_returns_error(self):
        ...
        result = await module._drive_move_file(file_id="any", new_parent_id="any")
