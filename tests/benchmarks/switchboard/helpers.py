...
BUTLER_REGISTRY: list[dict[str, Any]] = [
    ...
    },
    {
        "name": "general",
        "description": (
            "Flexible catch-all assistant for freeform data. IMPORTANT: This is a"
            " fallback butler, routing to this butler should be MUTUALLY EXCLUSIVE"
            " from routing to any other butler!"
        ),
...
# ---------------------------------------------------------------------------
# LLM call — OpenCodeAdapter (all models including Ollama go through OpenCode)
# ---------------------------------------------------------------------------
...
def _build_ollama_provider_config(model: str, ollama_url: str) -> dict[str, dict[str, Any]] | None:
    ...
    a /v1-suffixed base URL, and explicit model registrations.
    See https://docs.ollama.com/integrations/opencode
    """
    if not model.startswith("ollama/") or not ollama_url:
        return None
...
def call_routing(
    ...
    *,
    ...
    prompt and skill files, pointed at a mock MCP server. For ``ollama/*``
    models, injects the Ollama provider config so OpenCode can reach the
    Ollama server.
    ...
    """
    provider_config = _build_ollama_provider_config(model, ollama_url) if ollama_url else None
    adapter = OpenCodeAdapter(provider_config=provider_config)
