...
class TestDashboardCommand:
    ...
    def test_dashboard_help(self, runner):
        ...
        assert "41200" in result.output

    @patch("uvicorn.run")
    def test_dashboard_default_host_port(self, mock_uvicorn_run, runner):
        """Dashboard should use default host 0.0.0.0 and port 41200."""
    ...
    def test_dashboard_custom_host(self, mock_uvicorn_run, runner):
        ...
        assert result.exit_code == 0
        assert "Starting Butlers dashboard on 127.0.0.1:41200" in result.output
        mock_uvicorn_run.assert_called_once_with(
            "butlers.api.app:create_app",
            host="127.0.0.1",
            port=41200,
