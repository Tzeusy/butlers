...
_THREAD_TARGETED_NOTIFY_CHANNELS: frozenset[NotifyChannel] = frozenset({"telegram"})
...
class NotifyDeliveryV1(BaseModel):
    """Outbound delivery request details for `notify.v1`."""
...
class NotifyRequestV1(BaseModel):
    ...
    @model_validator(mode="after")
    def _validate_reply_context_requirements(self) -> NotifyRequestV1:
        ...
        if (
            self.delivery.channel in _THREAD_TARGETED_NOTIFY_CHANNELS
            and self.request_context.source_thread_identity is None
        ):
            raise PydanticCustomError(
                "missing_source_thread_identity",
                "notify.v1 reply intent requires request_context.source_thread_identity for "
                "channel '{channel}'.",
