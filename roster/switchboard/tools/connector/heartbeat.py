...
class ConnectorHeartbeatV1(BaseModel):
    ...
    capabilities: dict[str, Any] | None = Field(
        default=None,
        description='Optional connector capabilities (e.g. {"backfill": true}). '
        "Stored in connector_registry for feature-flag and routing decisions.",
    )
...
async def heartbeat(
    ...
    logger.info(
        ...
        "state=%s, uptime=%ds, capabilities=%s, deltas=%s",
        connector_type,
        endpoint_identity,
        instance_id,
        status.state,
