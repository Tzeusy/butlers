...
already archived / disconnected is left alone. The
``google_health:user:<owner>:<uuid>`` identity carries an account-specific UUID
suffix, so it is matched by its stable ``user:<owner>:`` prefix rather than the
volatile UUID. This is a data-fix migration (same pattern as sw_013's
``replay_safe=FALSE`` email seed); it deliberately does NOT delete anything.
...
#: identity, so re-running the migration is a no-op. The google_health
#: ``user:<owner>:<uuid>`` identity carries an account-specific UUID suffix, so
#: it is matched by its stable ``user:<owner>:`` prefix (LIKE) rather than the
#: volatile UUID. The literal prefixes contain no ``%``/``_`` wildcards, so no
...
_ARCHIVE_DEAD_IDENTITIES_SQL = [
    ...
    UPDATE connector_registry SET archived_at = now()
     WHERE archived_at IS NULL
       ...
       AND endpoint_identity = 'degraded'
    """,
    """
    UPDATE connector_registry SET archived_at = now()
     WHERE archived_at IS NULL
       AND connector_type = 'google_health'
       AND endpoint_identity LIKE 'user:uniquosity@gmail.com:%'
    """,
    """
    UPDATE connector_registry SET archived_at = now()
     WHERE archived_at IS NULL
    ...
    UPDATE connector_registry SET archived_at = now()
     WHERE archived_at IS NULL
       ...
       AND endpoint_identity = 'homeassistant.parrot-hen.ts.net:443'
    """,
]
