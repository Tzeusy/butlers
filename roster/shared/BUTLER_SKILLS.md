...
- **self-healing** — How to report unexpected errors for automated investigation via the `report_error`/`get_healing_status` tools. Only present when the `self_healing` module is enabled (`[modules.self_healing]` in your butler's `butler.toml`) for your butler; consult this skill when you encounter an exception you cannot resolve yourself.
