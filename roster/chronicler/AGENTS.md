...
- Own-schema writes only. Day-close synthesizes insights into `chronicler.*`
  memory; enrichment for other butlers is an MCP proposal, never a foreign write.
...
> spec is `openspec/changes/chronicler-intent-evidence-activity/` (design.md
> "Memory write-back loop"). Keep this runtime elaboration consistent.

The Chronicler imports the shared **memory module** (`[modules.memory]` in
`butler.toml`), so its facts live in the **`chronicler.*`** schema — writing
them satisfies the "own-schema only" invariant, not a foreign write. After the
day-close summary is produced, a deterministic completion-hook write-back
...
1. **Synthesized insights** — durable, *derived* facts with `source=chronicler`
   provenance, confidence, and decay (via `permanence`): sleep debt building,
   a lane skewing vs. the owner's usual, social cadence. Never raw evidence;
   never asserts attendance from a calendar (intent) block alone.
...
- Never write another butler's schema. Insights land in `chronicler.*`
  memory; enrichment for Relationship (or any butler) is an MCP proposal.
