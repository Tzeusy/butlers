...
<!--
Travel-local copy of the shared cross-butler-delegation skill
(roster/shared/skills/cross-butler-delegation/SKILL.md). Travel does not
`@include` roster/shared/AGENTS.md, so it carries this guidance locally
instead of via a symlink. Keep in sync by hand if the shared version changes.
-->
...
- Do not delegate a question your own domain can already answer — check your
  own tools/memory first (e.g. trip/document/itinerary questions belong to
  you, not another domain).
...
```python
delegate_ask(question="Does the user have a budget cap for international trips this quarter?")
```
