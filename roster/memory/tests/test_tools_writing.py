...
# Mock sentence_transformers before loading since embedding.py imports it.
# ---------------------------------------------------------------------------
...
def _load_tools_module():
    """Load tools.py with sentence_transformers mocked out."""
...
memory_store_episode = _tools.memory_store_episode
memory_store_fact = _tools.memory_store_fact
memory_store_rule = _tools.memory_store_rule
get_embedding_engine = _tools.get_embedding_engine
...
class TestMemoryStoreEpisode:
    ...
    async def test_delegates_to_storage(self, mock_pool: AsyncMock) -> None:
        """memory_store_episode delegates to _storage.store_episode."""
        episode_id = uuid.uuid4()
        expires_at = datetime.now(UTC)
        storage_result = {"id": episode_id, "expires_at": expires_at}

        with patch.object(_tools._storage, "store_episode", new_callable=AsyncMock) as mock_store:
    ...
    async def test_passes_session_id(self, mock_pool: AsyncMock) -> None:
        ...
        with patch.object(_tools._storage, "store_episode", new_callable=AsyncMock) as mock_store:
            ...
            await memory_store_episode(
                mock_pool, "test", "butler", session_id="sess-123"
            )
...
class TestMemoryStoreFact:
    ...
    async def test_delegates_to_storage(
        ...
        with patch.object(_tools._storage, "store_fact", new_callable=AsyncMock) as mock_store:
            mock_store.return_value = storage_result
...
class TestMemoryStoreRule:
    ...
    async def test_delegates_to_storage(
        ...
        with patch.object(_tools._storage, "store_rule", new_callable=AsyncMock) as mock_store:
            mock_store.return_value = storage_result
...
class TestGetEmbeddingEngine:
    ...
    def test_returns_engine_instance(self) -> None:
        ...
        _tools._embedding_engine = None
        with patch.object(
            _tools, "EmbeddingEngine", return_value=MagicMock()
        ) as mock_cls:
    ...
    def test_returns_same_instance_on_second_call(self) -> None:
        ...
        _tools._embedding_engine = None
        sentinel = MagicMock()
        with patch.object(_tools, "EmbeddingEngine", return_value=sentinel):
    ...
    def test_singleton_constructor_called_only_once(self) -> None:
        ...
        _tools._embedding_engine = None
        with patch.object(_tools, "EmbeddingEngine", return_value=MagicMock()) as mock_cls:
    ...
    def test_reset_singleton_allows_recreation(self) -> None:
        ...
        with patch.object(_tools, "EmbeddingEngine", return_value=first_sentinel):
            first = get_embedding_engine()

        _tools._embedding_engine = None

        with patch.object(_tools, "EmbeddingEngine", return_value=second_sentinel):
            second = get_embedding_engine()
