...
- **WHEN** a chat's participant count (resolved via `_get_participant_count`, cached with a 1-hour TTL) is known and `<= discretion_group_size_bypass_max` (env `TELEGRAM_USER_DISCRETION_GROUP_SIZE_BYPASS_MAX`, default 20), AND `_derive_chat_type` reports an actual group (`not in (None, "private")`)
...
- **AND** a DM (`chat_type == "private"`) never triggers the bypass either, even though DMs conventionally report `participant_count=2` — a 1:1 conversation is not "small group banter", and without this guard every DM would silently skip discretion regardless of sender trust
