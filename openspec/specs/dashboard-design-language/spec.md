...
Each butler SHALL have one assigned identity hue from `--category-1..8` (defined in
...
anywhere else. Canonical mapping:
...
("Run patrol", not "Force Patrol Now!"). Empty states are one serif-italic sentence with no
trailing explanation: *"Nothing waiting."*

#### Scenario: Empty state copy
- **WHEN** a surface has nothing to show
- **THEN** it renders a single serif-italic sentence with no explanatory paragraph and no illustration
...
state is one serif-italic sentence with no illustration; (9) headlines are sans 500, not bold;
(10) the page reads calm at 3am during an outage. When a page cannot be made to look like the
Overview, either the language is wrong or the page is — both are worth investigating before
shipping.
