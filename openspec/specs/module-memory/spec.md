...
without retaining that episode's raw content. Every local episode deletion MUST
atomically create or retain a tombstone containing only the episode identifier
and deletion timestamp. Facts and rules that name the deleted episode as
`source_episode_id` MUST retain that identifier; generic `memory_links` whose
source or target is that episode MUST remain resolvable as an expired source
relation. Tombstones MUST NOT contain episode content, title, prompt, runtime
output, credentials, or raw owner-message data.
