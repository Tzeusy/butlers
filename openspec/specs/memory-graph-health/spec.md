...
Defines the bounded, read-only graph-health coverage observation for memory
pools. It reports whether the dashboard has complete evidence, incomplete
evidence, or no completed evidence; it does not become a retention action,
repair path, provenance metric, or relationship-graph authority.
...
relationship entity-fact operation. Each completed relevant pool SHALL expose
its exact cleanup-lag numerator and denominator. Each genuinely unavailable
relevant pool SHALL remain explicitly unknown with nullable metrics. A
candidate pool that genuinely lacks a memory schema SHALL be absent rather than
