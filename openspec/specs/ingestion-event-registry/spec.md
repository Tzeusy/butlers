...
- **WHEN** `ingestion_event_sessions(pool, request_id)` is called with a UUID7 that has a corresponding `public.ingestion_events` row
...
- **WHEN** `ingestion_event_sessions(pool, request_id)` is called with a UUID7 minted for an internal session (no `public.ingestion_events` row)
