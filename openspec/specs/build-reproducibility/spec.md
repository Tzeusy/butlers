...
Defines the contract for reproducible builds and a pinned runtime toolchain. The
same source commit must yield the same dependency graph, toolchain, and services
regardless of when or where the build runs, so runtime behavior is attributable
to a commit rather than to silent upstream drift.
