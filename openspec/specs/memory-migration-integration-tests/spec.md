...
The memory migration integration tests spec defines requirements for real-DB integration tests that apply the full memory module migration chain to a PostgreSQL schema and verify the resulting schema matches what runtime code expects. These tests prevent code-vs-schema drift from going undetected.
