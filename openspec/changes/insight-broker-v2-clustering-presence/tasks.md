...
- [x] 1.4 Unit tests: entity grouping, time-window overlap, non-overlap, empty-window fail-open, explicit-window precedence over `event_date` (partial/empty/wrong-type), `event_date` normalization, transitive chains, no-correlation-data regression, malformed-metadata fail-open, determinism.
