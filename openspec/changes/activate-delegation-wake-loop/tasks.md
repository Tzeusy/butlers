...
- [ ] 1.1 Add the delegation-ledger representation, targeted grants, and
...
- [ ] 1.2 Update delegate_answer and the Switchboard route adapter to implement
...
- [ ] 1.3 Implement the trusted asker-side delegate_wake endpoint and
...
- [ ] 1.4 Add migrated-Postgres and focused route/tool tests for first-answer
...
- [ ] 1.5 Verify the implementation's MCP inventory against core-daemon:
...
- [ ] 2.1 Add the separately scoped runtime-config validation and configuration
  activation for the delegation group; preserve existing groups and do not
  alter ledger/callback semantics.
- [ ] 2.2 Activate and verify the intended non-staffer MCP inventory for
  ...
  Admission Boundary.
- [ ] 2.3 Add the shared prompt/guidance and local roster reachability checks
...
- [ ] 3.1 Implement one deterministic Relationship-to-Finance delegation seed
...
- [ ] 3.2 Prove that qualifying/no-birthday/duplicate/route-failure paths leave
  the primary briefing contribution correct and create no owner-facing egress.
- [ ] 3.3 Verify cross-butler-briefing-contribution: Delegated Return Wakes Are
...
- [ ] 4.1 Run the focused lint, format, migrated-DB, route, core-tool, and
...
- [ ] 4.2 Run scoped strict OpenSpec validation and a diff review confirming no
...
- [ ] 4.3 Complete bu-27dxl.5.5 reconciliation only after .5.2, .5.3, and .5.4
