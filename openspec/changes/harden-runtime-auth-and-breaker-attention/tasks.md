...
- [ ] 1.1 Add a guarded core migration for `public.runtime_attention_outbox`, immutable source snapshots, triggering-edge and direct-reissue unique constraints, fenced-claim fields/indexes, non-cascading retention semantics, Switchboard claim grants, and fixed-search-path `SECURITY DEFINER` producer operations with `PUBLIC` execution revoked before role-specific grants; write migration tests for upgraded, core-only, catalog-deletion, and role-enforced databases.
...
- [ ] 4.3 Add real-Postgres role-isolation and concurrency integration tests proving the authorized producer operation rejects forged source/payload data, `PUBLIC`/`connector_writer`/non-producer roles cannot execute it, producers have no raw outbox access, and Switchboard alone claims/delivers.
