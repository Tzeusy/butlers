...
- [ ] 3.3 Implement the deterministic Switchboard-owned runtime-probe coordinator behind a private authenticated dashboard control-plane command, not the generic MCP/LLM tool surface. It accepts only a catalog ID, uses the resolved catalog entry, shared runtime home, applicable authority, adapter construction, mapping, generated config, and runtime args, with bounded timeout, per-entry de-duplication, and global concurrency without domain MCP tools or routed provenance.
- [ ] 3.4 Route per-model Test, verify-all, and scheduled verification through the coordinator; preserve verification history semantics, return honest unavailable/rate-limited states, prove generic MCP clients cannot invoke it, and prove probe success never closes a breaker.
