...
| OpenCode and probe | adapter/API tests for canonical Go identities with native execution arguments, fail-closed owner control, asymmetric signed-control/generic-Secrets exclusion, replay denial, and probe-no-reset behavior | use an actual runtime probe plus a separate routed session; compare their independent evidence |
