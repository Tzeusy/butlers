...
- **WHEN** a CC instance completes successfully and the SDK response contains `input_tokens: 1500`, `output_tokens: 800`, and `model: "claude-sonnet-4-20250514"`
...
- **WHEN** a CC instance fails with an error but the SDK response includes partial token data (e.g., `input_tokens: 500` but no `output_tokens`)
...
- **WHEN** a CC instance is spawned and the SDK response indicates the model used was `"claude-sonnet-4-20250514"`
