...
Every butler daemon SHALL register a `notify(channel, message, recipient?)` MCP tool during startup. CC instances MUST be able to call this tool to send outbound notifications. The tool MUST be available in every butler's MCP tool surface regardless of which modules are enabled.

#### Scenario: Tool available to CC instance
- **WHEN** a CC instance spawned by a butler lists available MCP tools
...
The `notify` tool MUST route notifications through the Switchboard's `deliver()` tool via the butler daemon's MCP client connection. The call MUST be synchronous from the CC instance's perspective: the CC instance calls `notify()`, the butler daemon forwards to Switchboard `deliver()`, waits for the response, and returns the result to the CC instance.

#### Scenario: Successful notification delivery
- **WHEN** a CC instance calls `notify(channel='telegram', message='Hello')` and the Switchboard is reachable
- **THEN** the butler daemon MUST call the Switchboard's `deliver()` tool with the corresponding parameters and return a success result to the CC instance

#### Scenario: Synchronous behavior
- **WHEN** a CC instance calls `notify()`
- **THEN** the CC instance MUST block until the butler daemon receives a response from the Switchboard and returns it
...
If the Switchboard is unreachable or returns an error, the `notify()` tool MUST return an error result (not raise an exception). The error result MUST include a human-readable message describing the failure. The CC instance can then decide on fallback behavior (e.g., storing the message in state for later retry).

#### Scenario: Switchboard unreachable
- **WHEN** a CC instance calls `notify()` and the Switchboard connection is down or times out
...
- **WHEN** a CC instance calls `notify()` and the Switchboard's `deliver()` tool returns an error
...
- **THEN** the CC instance MUST be able to inspect the error and take alternative action (e.g., store the notification in the butler's state store for later retry)

---

### Requirement: Supported channel values
The `notify` tool MUST accept `channel` values of `'telegram'` and `'email'`. If a CC instance provides any other channel value, the tool MUST return an error result indicating the channel is unsupported.

#### Scenario: Valid telegram channel
- **WHEN** a CC instance calls `notify(channel='telegram', message='Test')`
- **THEN** the tool MUST accept the call and route it to the Switchboard

#### Scenario: Valid email channel
- **WHEN** a CC instance calls `notify(channel='email', message='Test')`
- **THEN** the tool MUST accept the call and route it to the Switchboard

#### Scenario: Invalid channel value
- **WHEN** a CC instance calls `notify(channel='sms', message='Test')`
...
- **WHEN** a CC instance calls `notify(channel='telegram', message='Alert')` without a `recipient` parameter
...
- **WHEN** a CC instance calls `notify(channel='email', message='Report', recipient='user@example.com')`
