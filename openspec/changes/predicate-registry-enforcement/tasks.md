...
## 6. memory_predicate_search MCP Tool

- [ ] 6.1 Add `memory_predicate_search(query, scope=None)` MCP tool in `__init__.py` — prefix match on name, text search on description, optional scope filter
- [ ] 6.2 Add underlying `predicate_search(pool, query, scope=None)` function in `tools/reading.py` or equivalent
- [ ] 6.3 Write unit tests: prefix search, description text search, empty query returns all, scope filter

## 7. Integration Tests and Audit

- [ ] 7.1 Audit all direct callers of `store_fact()` in `roster/*/tools/` — verify they pass correct `object_entity_id` for edge predicates and `valid_at` for temporal predicates
- [ ] 7.2 Run full memory module test suite — verify no regressions from registry enforcement (existing tests that use registered predicates must still pass)
- [ ] 7.3 Run full API test suite — verify dashboard endpoints still work
