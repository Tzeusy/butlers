...
- [ ] 3.7 Wire `tick` emission into the patrol loop in `src/butlers/modules/qa/__init__.py` (one event per non-terminal attempt per patrol cycle when no other events fired this cycle)
...
- [ ] 9.7 Implement `state_of_case(attempt) -> Literal["detect","diagnose","pr","landed","escalated"]` mapping the existing healing_attempts.status into the five-state vocabulary
