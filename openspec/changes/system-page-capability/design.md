...
- Specify the five ownership-fact domains as distinct API endpoints, each queryable independently.
...
### D1: Five ownership-fact domains, each a separate API endpoint.

The System page surfaces five domains: instance identity, database state, backup state, egress catalog, and per-butler heartbeats. Each domain maps to one `GET /api/system/<domain>` endpoint with its own Pydantic response model.
...
The `switchboard.dashboard_audit_log` table (written by `src/butlers/api/audit_emit.py` and `src/butlers/core/audit.py`) already records outbound operations with butler and operation metadata. The egress catalog groups these records by actor (resolved via the server-side actor registry from the `operation` field and `request_summary` JSONB) and computes `last_seen_at` and `total_calls`. No new write path is introduced.

Note: there is no `audit.events` table in the current codebase. All audit writes go to `switchboard.dashboard_audit_log`. The egress catalog must derive actor identity from the existing `operation` string and `request_summary` JSONB columns; the implementation bead must define the convention for identifying external-actor calls (e.g., a naming convention for `operation` values such as `"llm_api_call"`, `"telegram_send"`, etc.) before shipping the EgressCatalogTile.
...
In v1, the only contact that can view the egress catalog is the owner. The owner is identified by joining `public.contacts` to `public.entities` and asserting `'owner' = ANY(e.roles)` -- `public.contacts.roles` was dropped in migration `core_016`; roles now live exclusively on `public.entities.roles`. The forward path to listing other actors (family members, delegated access) is documented as an open question.
...
- **[Risk]** The egress catalog gives the owner a false sense of completeness if the audit log does not capture all outbound calls. **Mitigation**: the spec marks the egress catalog as "derived from the audit log" so the owner understands it is a view of recorded events, not a packet-capture guarantee. The `catalog_covers_from` field on the `EgressCatalog` response makes the window of coverage explicit (it is the timestamp of the oldest audit entry used to build the catalog), so the owner can see how far back the catalog reaches. Per-actor staleness is communicated via `last_seen_at` on each `EgressActor` entry.
...
- **[Q1 — Answered]** Does the audit log uniformly record all egress paths (LLM providers, Telegram, Gmail, Google APIs)? If coverage is incomplete, should the egress catalog show "recorded egress" with a caveat, or should the implementation bead first add missing audit calls before shipping the page?

  **Answer**: Ship the egress catalog as "recorded egress" with an explicit caveat. The `catalog_covers_from` field already communicates the audit window. The implementation bead (bu-ngfzz.2) MUST audit coverage for each egress path (LLM API, Telegram, Google Calendar, Gmail SMTP) at implementation time and file follow-up beads for any paths where `dashboard_audit_log` entries are absent. The EgressCatalogTile UI SHOULD include a visible footnote: "This catalog reflects data captured by the audit log. Coverage may be incomplete for some external services." Blocking on full coverage before shipping would delay a privacy-transparency feature that is valuable even with partial coverage; incremental improvement via follow-up beads is the right tradeoff.

- **[Q2 — Deferred to implementation]** How is backup recency determined? Is there a Minio/S3 bucket with timestamped snapshot objects, a `pg_dump` cron job writing to the filesystem, or something else? The implementation bead must discover the backup strategy and either spec the endpoint shape precisely or raise a follow-up bead.

  **Answer**: The backup strategy is not yet documented in this codebase. The implementation bead (bu-ngfzz.2) must discover the actual backup mechanism (filesystem pg_dump, Minio/S3, or none) and document it in `AGENTS.md`. The `/api/system/backups` endpoint shape (as specced) is intentionally resilient: `backup_source_reachable: false` covers the case where no backup source is found. If no backup strategy exists, the endpoint returns `last_backup_at: null, backup_source_reachable: false` and the tile renders a "No backup configured" state. The implementation bead should raise a separate bead for configuring backup if none is found.

- **[Q3 — Answered]** Should database growth rate be shown in v1? If yes, the implementation bead must introduce a periodic snapshot mechanism. If no, the tile can show current size only and defer growth to a follow-up.

  **Answer**: Defer growth rate to v2. The v1 tile shows current size only (`total_size_bytes`, per-schema breakdown, and largest tables). `growth_rate_bytes_per_day: null` is already specced as the v1 placeholder. Growth rate requires a periodic snapshot table (`{schema}.system_snapshots`) populated by a scheduled job -- that is a new migration and new infrastructure. The value of showing current size is immediate and high; the incremental value of growth rate does not justify the infrastructure investment in v1.

- **[Q4 — Answered]** The forward path for multi-viewer access to the egress catalog (Settled Direction #1: "close family members later"). When family-member access lands, should the egress catalog be hidden entirely from non-owner viewers, or should it show only the facts relevant to data the viewer has access to?

  **Answer**: Hide entirely from non-owner viewers in v2. The egress catalog is a system-level privacy surface, not a per-viewer data surface. It shows which external services have processed data from this instance -- that is an owner-operator fact, not a per-contact fact. A family member with delegated access to a specific domain (e.g., calendar data) should not be able to see that the instance also sent data to a Telegram API or an LLM provider. When multi-viewer access is designed, the egress catalog MUST require an explicit owner-only capability gate (not just "authenticated dashboard session"). This answer resolves the forward-path question: no partial view of the egress catalog is appropriate for non-owner contacts; the endpoint stays 403 for them until a separate spec change argues otherwise.
