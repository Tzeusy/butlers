...
- [ ] 1.1 Approvals-chain migration `approvals_003`: add `blast_radius TEXT` and `reversibility TEXT` (nullable, CHECK enums) to `pending_actions`; legacy rows readable
- [ ] 1.2 Extend gate metadata extraction: accept `_blast_radius`/`_reversibility`, validate enums; typed-evidence entries `{type, ref, note}` with legacy-string coercion
...
- [ ] 3.2 Callback token mint/verify helper (HMAC-SHA256 over action_id+verb+requested_at, daemon-internal key, 64-byte-safe format)
