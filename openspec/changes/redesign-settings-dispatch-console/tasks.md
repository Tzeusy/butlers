...
- [ ] 5.0 **Rename `src/butlers/api/routers/costs.py` to `src/butlers/api/routers/spend.py`** with a **90-day deprecation period per design.md §D18**. Dual-mount: BOTH `/api/costs/*` (existing) AND `/api/spend/*` (new) point to the same handlers. `/api/costs/*` responses include `Deprecation: true` and `Sunset: <date 90 days out>` headers per RFC 8594. Migrate path mapping: `/api/costs/summary` ↔ `/api/spend?period=`, `/api/costs/daily` ↔ `/api/spend/daily`, `/api/costs/top-sessions` ↔ `/api/spend/top-sessions`, `/api/costs/by-schedule` ↔ `/api/spend/by-schedule`. Update frontend hook to consume `/api/spend/*` immediately. File a follow-up bead to delete `/api/costs/*` after 90 days.
