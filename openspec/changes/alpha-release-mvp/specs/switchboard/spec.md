...
### Requirement: Scheduled Execution Mode Contract
Switchboard scheduled tasks use dual-mode dispatch.

#### Scenario: Native mode scheduled tasks
- **WHEN** a schedule has a native handler (e.g., `eligibility-sweep`, `connector-stats-hourly-rollup`, `connector-stats-daily-rollup`, `connector-stats-pruning`)
- **THEN** execution runs directly against the Switchboard DB pool using `dispatch_mode=job`
...
The switchboard butler has specific runtime and module configuration.
...
- **WHEN** the switchboard butler starts
- **THEN** the following modules are enabled: calendar (Google provider), telegram (bot mode), memory, email (bot mode)
