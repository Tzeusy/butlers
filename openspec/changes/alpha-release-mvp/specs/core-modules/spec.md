...
Non-messenger butlers are prohibited from registering channel egress tools (matching `(user|bot)_<channel>_(send_message|reply_to_message|send_email|reply_to_thread)`). A `ChannelEgressOwnershipError` is raised if a non-messenger butler attempts this.

#### Scenario: Non-messenger egress tool rejected
- **WHEN** a non-messenger butler's module registers a tool matching the channel egress pattern
