...
The module registers 16 MCP tools total. The core CRUD tools are: `calendar_list_events`, `calendar_get_event`, `calendar_create_event`, `calendar_update_event`, `calendar_delete_event`. A caller MAY pass an explicit `calendar_id` to target a specific calendar; the available `calendar_id` values SHALL be enumerated by the separate read-only `calendar_list_calendars` tool (see the Calendar Source Listing Tool requirement). The "16 MCP tools total" count is NOT re-pinned by this change — adding `calendar_list_calendars` changes the registered tool surface, but the canonical total is reconciled by the in-flight `calendar-recurrence-scope-editing` change that owns the count re-pin.
