...
- [x] 1.1 Add a `calendar_list_calendars` MCP tool wrapping the existing `provider.list_calendars()`
- [x] 1.2 Normalize each entry to `{calendar_id, summary, primary, access_role, is_butlers_calendar, selectable}`; flag `_resolved_calendar_id` as the Butlers calendar and mark non-writer/owner calendars as not selectable
- [x] 1.3 Fail-open: provider errors return an empty list with error metadata rather than raising
- [x] 1.4 Re-pin the normative MCP tool count in the `Calendar Event CRUD Tools` requirement from 20 to 21 (PR #2620 reconciled the surface to 20; `calendar_list_calendars` adds exactly one) and refresh the enumerated tool list
- [x] 1.5 Unit tests: Butlers calendar flagged; read-only calendar non-selectable; provider failure fails open
...
- [x] 5.1 Add `GET /api/calendar/accounts` returning `public.google_accounts` joined with the Google Calendar connector per-account health (status, error_kind, last ingest)
- [x] 5.2 Add request/response models in `src/butlers/api/models/calendar_workspace.py`
- [x] 5.3 Read-only: this endpoint does NOT connect/disconnect accounts (that stays in the Google accounts surface)
- [x] 5.4 API tests: returns connected accounts with health; degrades gracefully when connector health is unavailable

## 6. Per-calendar source enable/disable (bu-6cf3ri)

- [x] 6.1 Add `POST /api/calendar/sources` to enable/disable a single calendar as a sync source by toggling state on the existing `calendar_sources` row (NO new table)
- [x] 6.2 A disabled source is skipped by the sync loop and rendered as off (not failed) in the sources drawer
- [x] 6.3 API tests: toggling a source updates its enabled state; a disabled source is skipped on the next sync
