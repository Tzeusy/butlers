...
- Specify all core components: state store (KV JSONB), task scheduler (cron-driven with TOML sync), CC spawner (locked-down ephemeral Claude Code instances), session log
...
- `session-log`: Logs every CC invocation — prompt, tool calls, outcome, duration, trace ID. Tools: `sessions_list`, `sessions_get`.
