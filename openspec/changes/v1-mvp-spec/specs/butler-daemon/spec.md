...
THEN the daemon SHALL raise a connection error and MUST NOT proceed to migration or server startup.

---

### Requirement: Core Migration Application

After database provisioning, the daemon SHALL apply core database migrations. Core migrations create the shared tables that every butler requires: `state`, `scheduled_tasks`, and `sessions`.

Migrations SHALL be plain SQL files located in `migrations/core/`, applied in lexicographic order. The daemon SHALL maintain a `_migrations` tracking table to record which migrations have been applied. A migration that has already been applied SHALL NOT be re-applied.

#### Scenario: Core tables created on first startup

WHEN the daemon starts against an empty database with no `_migrations` table,
THEN the daemon SHALL create the `_migrations` tracking table,
AND the daemon SHALL apply all SQL files from `migrations/core/` in lexicographic order,
AND after migration the database SHALL contain the `state`, `scheduled_tasks`, and `sessions` tables,
AND each applied migration SHALL be recorded in the `_migrations` table.

#### Scenario: Already-applied migrations are skipped

WHEN the daemon starts and the `_migrations` table already records `001_core.sql` as applied,
THEN the daemon SHALL NOT re-apply `001_core.sql`,
AND the daemon SHALL apply any newer migration files not yet recorded in `_migrations`.

#### Scenario: Migration failure halts startup

WHEN a core migration SQL file contains a syntax error or fails to execute,
...
After core migrations, the daemon SHALL apply butler-specific migrations if a `migrations/<butler-name>/` directory exists. These migrations create tables specific to the butler's domain (e.g., `contacts` for the Relationship butler, `measurements` for the Health butler).

Butler-specific migrations follow the same rules as core migrations: plain SQL files, lexicographic order, tracked in `_migrations`, idempotent on re-run.

#### Scenario: Butler-specific migrations applied after core

WHEN the daemon starts for the Relationship butler and `migrations/relationship/001_relationship.sql` exists,
THEN the daemon SHALL first apply all core migrations,
AND THEN apply `migrations/relationship/001_relationship.sql`,
AND both core and butler-specific migrations SHALL be recorded in `_migrations`.

#### Scenario: No butler-specific migrations directory

WHEN the daemon starts for a butler named "minimal" and no `migrations/minimal/` directory exists,
...
3. Call `migrations()` and apply any module-specific migrations.
...
AND apply the email module's migrations,
...
4. Apply core migrations.
5. Apply butler-specific migrations.
...
WHEN the daemon starts with a valid config, accessible database, clean migrations, and valid modules,
...
#### Scenario: Migration failure prevents tool registration

WHEN the daemon fails during step 4 (core migrations),
THEN steps 5 through 9 SHALL NOT execute,
AND the daemon SHALL report the migration error.
...
THEN the Butler SHALL execute the full startup sequence (config already loaded, provision DB, apply migrations, load modules, register tools, start server) in the specified order.
...
During startup, after migrations are applied and before the server starts, the daemon SHALL synchronize scheduled tasks defined in `[[butler.schedule]]` sections of `butler.toml` to the `scheduled_tasks` database table.
...
THEN the log output SHALL contain entries for config loaded, database connected, migrations applied, modules initialized, and server started.
