...
DND is no longer active, and its uncommitted reservations have been released;
this preserves the requirement for fresh owner intent after DND. A stale worker
whose fence no longer matches cannot prepare, commit, abort, or release.
...
second release action. An explicit abort may release uncommitted reservations
only into a durable retained/abandoned recovery state, never into a silent
partial-send path.
...
| `abort.v1` | Switchboard → prepared participants | Releases only an uncommitted local reservation, or records a durable retained/block reason | It never silently converts a committed cohort into ordinary scheduler work. |
...
| `release_ready` | all commit receipts persisted | `egress_sending`, `egress_delivered`, `egress_ambiguous` | Every selected row remains bound to this action key. |
...
| `egress_delivered` | provider receipt is durable | terminal | All committed rows have one delivered/coalesced audit outcome. |
| `egress_ambiguous` | post-start result is unknown | terminal until explicit reconciliation | No automatic egress retry is allowed. |
| `blocked_dnd` | DND wins a guarded transition | explicit abort, then only a new accepted direct DM after DND clears may create a higher-fence successor | No partial send or generic context mutation occurs. |
| `retained_*` | unavailable participant, oversize, target mismatch, or commit failure | same-fence recovery only where its persisted cutoffs/manifest permit; otherwise explicit retained abort | No partial send; late rows never join the retained cohort. |
...
`release_prepared`, `release_committed`, or `egress_ambiguous` as ordinary
`pending` work; an explicit recovery procedure either resumes the new protocol
or retains the cohort until a compatible binary returns.
