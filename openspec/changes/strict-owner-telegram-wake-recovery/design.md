...
| `abort.v1` | Switchboard → prepared participants | Persists a reason-specific replay-safe outcome. Only an ordinary all-pre-commit cancellation can move its whole cohort to `pending`; DND and retained outcomes remain protocol-bound. | It never silently converts a blocked, retained, committed, delivered, or ambiguous cohort into ordinary scheduler work. |
...
states atomically before releasing any reservation. The only
scheduler-visible abort is an explicit `ordinary_precommit_cancel`: every
selected row is still `release_prepared`, no participant has reported DND,
unavailable, oversize, mismatch, or commit failure, and no `commit.v1`, egress
intent, or send-start marker exists. That one transition records
`aborted_precommit`, returns the whole cohort to `pending` under the current
fence audit, and lets the ordinary scheduler use the stored delivery path. A
same-fence replay returns that recorded cancellation; it cannot restart the
old run.
...
| `claimed` | owner/window CAS claim succeeds | `preparing`, `blocked_dnd`, `retained_unavailable` | One current fence owns the window. |
| `preparing` | all participant prepare calls started | `prepared`, `blocked_dnd`, `retained_unavailable`, `retained_oversize`, `retained_mismatch` | Prepared rows are scheduler-ineligible. |
| `prepared` | all participants supplied a compatible snapshot | `committing`, `blocked_dnd` | Composition manifest and exact target are immutable. |
...
| Generic abort loses a safety-bound cohort | Reason-specific durable abort records plus scheduler exclusion | Only audited `aborted_precommit` rows can be `pending`; DND, retained, committed, delivered, and ambiguous rows remain protocol-bound. |
...
| Reason-specific abort fencing | Transition tests persist and replay each abort reason with run, row, fence, and scheduler assertions | Only an all-pre-commit ordinary cancellation returns its complete cohort to `pending`; DND requires a later qualifying DM after DND clears, retained cohorts preserve all-or-nothing recovery, and committed/delivered/ambiguous cohorts never fall back to the scheduler. |
...
`release_prepared`, `release_retained_*`, `release_committed`, `aborted_dnd`,
`aborted_retained`, or `egress_ambiguous` as ordinary `pending` work; an
explicit recovery procedure either resumes the new protocol or retains the
cohort until a compatible binary returns.
