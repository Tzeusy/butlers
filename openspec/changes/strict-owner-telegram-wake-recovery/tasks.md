...
- [ ] 3.2 Implement origin-local prepare, deterministic cutoff, same-fence replay, abort, and commit transitions without exposing queue tables to Switchboard.
- [ ] 3.3 Restrict the ordinary deferred scheduler to pending rows and test prepared, committed, legacy, context-only, retry, and post-cutoff late-row behavior without re-gating stored holds.
...
- [ ] 4.3 Add concurrency tests for DND before prepare/commit and Messenger admission, exact-target mismatch, unavailable participant, oversize cohort, and the all-zero empty terminal result.
...
- [ ] 5.4 Add fault-injection tests for coordinator, origin, Health, and Messenger crashes at each durable state, including no second provider send after an uncertain post-start outcome.
...
- [ ] 6.2 Add end-to-end tests for duplicate accepted events, window fencing, exact-target one-message composition, late rows, DND races, all-or-nothing retention, and receipt/ambiguity recovery.
