...
The OAuth authorization request SHALL include exactly the scopes needed for connector operation.

#### Scenario: Scope specification

- **WHEN** the OAuth authorization URL is constructed
- **THEN** the `scope` parameter SHALL include:
  - `user-read-playback-state` — allows reading current playback state (track, device, context)
  - `user-read-recently-played` — allows reading recently played tracks with timestamps
  - `user-top-read` — allows reading top artists and tracks for preference profiling
- **AND** no additional scopes SHALL be requested (principle of least privilege)
