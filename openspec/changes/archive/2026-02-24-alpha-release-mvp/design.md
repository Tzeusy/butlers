...
The system is decomposed into 38 capability specs across 6 domains:
...
**Connectors (5 specs):**
- `connector-base-spec` — Shared interface contract: ingest.v1 envelope schema, deduplication strategy, CachedMCPClient transport, heartbeat protocol, liveness/eligibility, statistics rollups, rate limiting, Prometheus metrics, horizontal scaling
...
- `butler-base-spec` — Shared roster conventions, directory structure, butler.toml schema, port assignment
- `butler-switchboard` — Full routing control plane: ingestion, LLM-driven decomposition, registry, lifecycle states, error taxonomy, backfill, DurableBuffer, priority queuing, butler configuration, scheduled tasks, skills
...
Considered grouping all core specs into one file, but individual files enable independent sync and easier review. 38 files is manageable and matches the system's natural capability boundaries.
