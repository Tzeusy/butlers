...
sidebar Main section with a dedicated icon and a badge that shows the number
of currently open decision records when greater than zero. The badge SHALL
remain `0` when the decision export is unavailable rather than asserting a
calm count.
...
- **AND** a positive open-decision count appears as its badge
