...
Current core migration state: public.entities exists; public.memory_catalog does not.
...
- Create migration for public.memory_catalog to back existing runtime code
...
### D4: public.memory_catalog as memory module migration (mem_022)

Create the table via a memory migration using schema-qualified `public.memory_catalog` DDL, matching the columns already referenced in `_upsert_catalog()` and `memory_catalog_search()`. This avoids a dependency on the external core migration system.
...
**[Risk: public.memory_catalog migration runs in wrong schema context]**
→ Mitigation: Use fully-qualified `public.memory_catalog` in DDL. Add IF NOT EXISTS guard. The migration is idempotent.
