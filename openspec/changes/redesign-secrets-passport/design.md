...
| Q7 | Brief §5 | Spine `needs-hand` pin: backend tag vs client-side compute | **defer-to-implementation** | Default: client-side derive from `state != ok` so the backend `/inventory` endpoint stays shape-stable. Beads-writer may flag this for revisitation if performance evidence emerges. |
...
| Q9 | Brief §5 | Tweaks persistence mechanism (localStorage vs URL fragment vs server-side prefs) | **defer-to-implementation** | Default per `butler-secrets §Tweaks-Panel State Persistence`: match whichever pattern the in-flight ingestion-redesign or entity-redesign ships first; fall back to `localStorage` keyed `secrets.tweaks.*`. |
...
2. **Migration order:**
   ...
   - Phase D (frontend): passport-book components, spine, page, tweaks, drawers, deep-link routing, identity switcher.
