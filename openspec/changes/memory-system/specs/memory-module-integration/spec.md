...
### Requirement: Memory module-driven context injection before runtime spawn

When a butler has memory module enabled, the LLM CLI spawner SHALL call `memory_context(trigger_prompt, butler_name)` on that same butler MCP server before spawning a runtime instance. Calls SHALL propagate authenticated request context so memory retrieval can enforce tenant and lineage boundaries. The returned memory block SHALL be injected into the system prompt after `CLAUDE.md`.

#### Scenario: runtime instance receives memory context
- **WHEN** the health butler (with memory module enabled) spawns a runtime instance for prompt "Help user log weight"
...
- **THEN** runtime spawn SHALL continue without memory context
- **AND** a warning SHALL be logged

### Requirement: Episode storage after runtime session completion

After every runtime session completes, when memory module is enabled, the daemon SHALL call `memory_store_episode` with key observations, butler identity, and session linkage. Tenant identity SHALL come from authenticated request context.
...
#### Scenario: runtime instance calls memory_recall mid-session
- **WHEN** a runtime instance for butler `health` calls `memory_recall(topic="user medications")`
...
- **AND** runtime spawn/session completion SHALL proceed without memory hook calls
