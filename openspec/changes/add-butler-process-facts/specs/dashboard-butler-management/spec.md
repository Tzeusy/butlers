...
The Butler detail Overview tab SHALL render a process facts card that displays stable, container-boundary-safe runtime facts for the selected butler. The card SHALL include `container_name`, `port`, `uptime`, and `config_path`, and it SHALL NOT include `pid` or any process ID field.
...
#### Scenario: Uptime is shown from liveness registry timestamps
- **WHEN** the Overview tab renders process facts for a butler with switchboard liveness data
- **THEN** the card SHALL show `uptime` as a human-readable duration derived from existing liveness registry timestamps
- **AND** the source SHALL include the heartbeat/system router's registry read of `butler_registry.last_seen_at` (`src/butlers/api/routers/system.py:643-699`)
...
- **AND** tests SHALL assert the word `pid` is absent from the rendered card DOM
