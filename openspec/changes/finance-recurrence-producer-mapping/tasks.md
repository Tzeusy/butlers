...
- [ ] 2.1 Extend connector measurability and persistence to require the
...
- [ ] 2.2 Transition existing Health call sites and rows: preserve null
...
- [ ] 2.3 Add migrated-PostgreSQL schema/helper/Health tests plus the
  dead-endpoint/healthy-sibling matrix in both row orders.

## 3. Finance runtime adoption after the shared prerequisite

- [ ] 3.1 Persist `finance:recurrence:{recurring-group-id}` and
...
- [ ] 3.2 Keep SimpleFIN and all unsupported/mixed/unprovable sources unmeasurable until an
  independently approved producer-liveness contract exists.
- [ ] 3.3 Keep declared renewal dates distinct from expected transaction observations and preserve
  the current forward-looking reminder path.

## 4. Consumers and presentation

- [ ] 4.1 Expose Finance recurrence measurability without direct cross-schema reads or inferred
  payment-state fields.
- [ ] 4.2 Ensure APIs, subscription audit, the Finance tab, and proactive output never turn
  unmeasurable or absent-without-policy into missed-renewal, payment, cancellation, paused, or
  stopped wording.

## 5. Verification

- [ ] 5.1 Add migrated-PostgreSQL tests that kill the exact Gmail endpoint after
...
- [ ] 5.2 Test stale, dead/offline, degraded, paused, missing, unreadable, mixed, SimpleFIN,
  manual/import, source-message-only, backfill, split, and legacy provenance.
- [ ] 5.3 Prove healthy/current Gmail and attested owner sources produce the RFC 0029 state while
  no unapproved missed-recurrence candidate is emitted.
- [ ] 5.4 Prove existing forward-looking annual renewal and predicted-bill policies retain their
  current windows, categories, priorities, deduplication, cooldown, expiry, and wording.
- [ ] 5.5 Keep an executable tool-registration guard for `track_subscription_fact` and its
